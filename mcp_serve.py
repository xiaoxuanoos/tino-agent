"""
Tino MCP Server — expose messaging conversations as MCP tools (`hermes mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Tino-specific
...
def _hermes_home() -> Path:
    try:
        ...
        return Path(os.environ.get("TINO_HOME", Path.home() / ".hermes"))
...
class EventBridge:
    ...
    event queue with waiter support (the Tino analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Tino MCP server with all tools registered."""
    ...
    mcp = MCPServer("hermes", instructions=(
        "Tino Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Tino MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:

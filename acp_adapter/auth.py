"""ACP auth helpers — detect and advertise Tino authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Tino runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Tino.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Tino credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Tino using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Tino provider", type="terminal", args=["--setup"],
        description=("Open Tino's interactive model/provider setup in a terminal. "
                     "Use this when Tino has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Tino can resolve any runtime provider credentials."""

...
# (TINO_PYTHON is exported by the devShell hook and ships [dev] extras:
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${TINO_PYTHON:-}" ] && [ -x "$TINO_PYTHON" ] \
    && "$TINO_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: TINO_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `hermes` binary rather
  # than the devShell hook.
  PYTHON="$TINO_PYTHON"
  echo "▶ no local venv — using Nix dev venv via TINO_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and TINO_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
#
#   * TINO_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * TINO_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     TINO_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# TINO_RUN_SLOW_PET_TESTS / TINO_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no TINO_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in TINO_TEST_IMAGE TINO_TEST_WORKERS TINO_TEST_PATHS \
  TINO_TEST_FILE_TIMEOUT TINO_TEST_FILE_RETRIES TINO_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${TINO_RUN_SLOW_PET_TESTS:+TINO_RUN_SLOW_PET_TESTS="$TINO_RUN_SLOW_PET_TESTS"} \
  ${TINO_E2E_BROWSER:+TINO_E2E_BROWSER="$TINO_E2E_BROWSER"} \

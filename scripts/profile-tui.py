#!/usr/bin/env python3
"""Drive the Tino TUI under TINO_DEV_PERF and summarize the pipeline.
...
launches `node dist/entry.js` directly with TINO_TUI_RESUME set so it
bypasses the hermes_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  TINO_PERF_LOG     (default ~/.hermes/perf.log)
  TINO_PERF_NODE    (default node from $PATH)
  TINO_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests TINO_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("TINO_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".hermes"

DEFAULT_TUI_DIR = Path(
    os.environ.get("TINO_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("TINO_PERF_LOG", str(get_hermes_home() / "perf.log")))
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — TINO_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["TINO_DEV_PERF"] = "1"
    env["TINO_DEV_PERF_MS"] = str(args.threshold_ms)
    env["TINO_DEV_PERF_LOG"] = str(log)
    env["TINO_TUI_RESUME"] = sid
    ...
    node = os.environ.get("TINO_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="TINO_DEV_PERF_MS (0 = capture all)")

...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:TINO_HOME) { $env:TINO_HOME } else { "$env:LOCALAPPDATA\hermes" }),
    [string]$InstallDir = $(if ($env:TINO_HOME) { "$env:TINO_HOME\hermes-agent" } else { "$env:LOCALAPPDATA\hermes\hermes-agent" }),
...
# TINO_HOME and InstallDir -- in that short form:
...
if ($PSBoundParameters.ContainsKey('HermesHome')) {
    ...
    $HermesHome = ConvertTo-LongPath $(
        if ($env:TINO_HOME) { $env:TINO_HOME } else { "$env:LOCALAPPDATA\hermes" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:TINO_HOME) { "$env:TINO_HOME\hermes-agent" } else { "$env:LOCALAPPDATA\hermes\hermes-agent" }
    )
}
...
function Resolve-UvCmd {
    ...
    # sibling process and TINO_HOME wasn't propagated).
...
function Install-Git {
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    ``TINO_GIT_BASH_PATH`` (User scope) so Hermes can find it in a fresh
...
function Set-GitBashEnvVar {
    ...
    ``TINO_GIT_BASH_PATH`` (User env scope) so Hermes can find it even before
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("TINO_GIT_BASH_PATH", $candidate, "User")
            $env:TINO_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set TINO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Hermes may not find Git Bash."
    Write-Info "If needed, set TINO_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Set-PathVariable {
    ...
    # Set TINO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\hermes instead
    # of the Unix default ~/.hermes
    $currentHermesHome = [Environment]::GetEnvironmentVariable("TINO_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("TINO_HOME", $HermesHome, "User")
        Write-Success "Set TINO_HOME=$HermesHome"
    }
...
function Copy-ConfigTemplates {
    ...
    # Create the TINO_HOME directory structure ($HermesHome, default %LOCALAPPDATA%\hermes)

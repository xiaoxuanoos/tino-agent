...
When Tino runs in TUI mode, the gateway child process communicates with
...
# gated behind ``TINO_ENABLE_PROJECT_PLUGINS``) — see
...
def main() -> int:
    ...
    if os.environ.get("TINO_ENABLE_PROJECT_PLUGINS"):

...
# Python-driven tool session (e.g. Tino terminal tool). A pre-set PYTHONPATH
...
if [ -n "${TINO_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$TINO_INSTALL_DIR"
...
#   data still at /root/.hermes (TINO_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$TINO_HOME/.no-bundled-skills so future"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Tino.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.hermes/hermes-agent"
            echo "                   default (root, Linux): /usr/local/lib/hermes-agent"
            echo "  --hermes-home PATH  Data directory (default: ~/.hermes, or \$TINO_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Tino installs the code under"
            echo "  /usr/local/lib/hermes-agent and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$TINO_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$TINO_HOME/hermes-agent are preserved in-place."
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Tino-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Tino Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $TINO_HOME/hermes-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $TINO_HOME/hermes-agent
...
#                              $TINO_HOME/hermes-agent — then preserve it)
#
# Always no-op when the user set --dir or $TINO_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in TINO_HOME.
    if is_termux; then
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$TINO_HOME/hermes-agent/.git" ]; then
            INSTALL_DIR="$TINO_HOME/hermes-agent"
        ...
        log_info "  Data:    $TINO_HOME (unchanged)"
...
# Point a Tino-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $TINO_HOME/node/bin — which is NOT on
...
# Tino's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Tino-managed npm, so calling it on
...
configure_managed_node_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$TINO_HOME/node/etc/npmrc"
}
...
install_uv() {
    ...
    # Tino owns its own uv at $TINO_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `hermes update` stay in sync.
    local _managed_uv="$TINO_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $TINO_HOME/bin ..."
    mkdir -p "$TINO_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "${TMPDIR:-$TINO_HOME}/hermes-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "${TMPDIR:-$TINO_HOME}/hermes-uv-installer.$$.sh")"
    ...
    # directly into $TINO_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$TINO_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Tino currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Tino requires Python >=3.11,<3.14"
...
# replaced with the Tino-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Tino-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Tino-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Tino-managed Node from a previous run over a too-old system one.
    if [ -x "$TINO_HOME/node/bin/node" ] && [ -x "$TINO_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$TINO_HOME/node/bin/node" --version)"; then
        export PATH="$TINO_HOME/node/bin:$PATH"
        log_success "Node.js $("$TINO_HOME/node/bin/node" --version) found (Tino-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Tino-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Tino requires Node 22.22+, 24.11+, or 26+) — installing Tino-managed Node $NODE_VERSION..."
...
install_node_line() {
    ...
    mkdir -p "$TINO_HOME"
    mv "$extracted_dir" "$TINO_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$TINO_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$TINO_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$TINO_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$TINO_HOME/node/bin/node" --version 2>&1); then
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Tino install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Tino Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Tino behaves unexpectedly."
                    else
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        TINO_BIN="$INSTALL_DIR/venv/bin/python"
        TINO_ENTRYPOINT="$INSTALL_DIR/hermes"
    else
        TINO_BIN="$(which hermes 2>/dev/null || echo "")"
        if [ -z "$TINO_BIN" ]; then
    ...
    if [ ! -x "$TINO_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$TINO_ENTRYPOINT" ]; }; then
        log_warn "Tino launcher prerequisites not found"
    ...
    # Older installs created this path as a symlink to $TINO_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$TINO_BIN"` self-recurse. (#21454)
...
unset PYTHONPATH
unset PYTHONHOME
exec "$TINO_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/hermes-agent" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    # this launcher those hosts report Tino as not installed. (#21454 applies
...
unset PYTHONPATH
unset PYTHONHOME
exec "$TINO_BIN" "$TINO_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/hermes-acp" <<EOF
#!/usr/bin/env bash
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Tino Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    mkdir -p "$TINO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.hermes/.env (top level, easy to find)
    if [ ! -f "$TINO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$TINO_HOME/.env"
    ...
    chmod 600 "$TINO_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.hermes/config.yaml (top level, easy to find)
    if [ ! -f "$TINO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$TINO_HOME/config.yaml"
    ...
    if [ ! -f "$TINO_HOME/SOUL.md" ]; then
        cat > "$TINO_HOME/SOUL.md" << 'SOUL_EOF'
You are Tino Agent, built by Nous Research. Be direct: match the length of your reply to the weight of the ask — a one-line question gets a one-line answer, and finished work gets a short report of what changed, what's verified, and what's left, never a replay of the process. No filler ("Great question," "I'd be happy to"), no restating the request back, no re-summarizing what you already said, no narrating tool calls the user can see. Plain claims over adjectives; when unsure, say so plainly. Agree because it's right, not because the user said it. Depth is earned — give it when the user asks for detail, teaches, or the stakes demand it, not by default.
SOUL_EOF
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        printf '%s\n' \
            ...
            > "$TINO_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $TINO_HOME/.no-bundled-skills"
        ...
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            ...
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$TINO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$TINO_HOME/skills/" 2>/dev/null || true
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Tino Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Tino will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Tino Agent browser tools — explicit browser override."
...
install_computer_use_driver() {
    ...
    # so a stalled GitHub download can't hang the Tino install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Tino'
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Tino to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$TINO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                TINO_CMD="$(get_hermes_command_path)"
                $TINO_CMD whatsapp || true
            fi
        else
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $TINO_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $TINO_CMD gateway start 2>/dev/null; then
            ...
            nohup $TINO_CMD gateway > "$TINO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
...
write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.hermes-bootstrap-complete, which tells the Tino
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $TINO_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $TINO_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $TINO_HOME/cron/, sessions/, logs/"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$TINO_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$TINO_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$TINO_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$TINO_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Tino` rename
...
install_desktop() {
    ...
    # supported Node lines and prepends the Tino-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Tino" ]; then
            app="$desktop_dir/release/linux-unpacked/Tino"
        ...
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Tino.app" \
            "$desktop_dir/release/mac/Tino.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # importing Python cannot reverse it by loading TINO_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if TINO_HOME="$TINO_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Tino-managed Node
            # provisioned during prerequisites/node-deps (at $TINO_HOME/node/bin)
        ...
        complete)
            ...
            # $TINO_HOME. $TINO_HOME is a shared data dir (it can be
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $TINO_HOME.
    # $TINO_HOME is a shared data dir (it can be bind-mounted into a Docker

...
# (hermes_<name>), map back in normalize_response so Tino dispatch is unaffected. See #85589.
...
def _openai_prefers_native_web_search() -> bool:
    ...
    Tino tool in place rather than swap in a built-in the endpoint might reject.
...
def _alias_wire_tools(
    ...
    swaps it 1:1 for the built-in, client mode keeps Tino dispatch under an alias.
    ...
    # Keep Tino's client-side functions available under wire aliases.
    ...
    # xAI server-side web search vs Tino web providers. grok models on xAI's /v1/responses surface have a
    ...
    # or resolved): keep Tino dispatch so ``web.backend`` / ``web.search_backend`` is honored, but rename
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        kwargs = {
            # ``-900k`` picker variants are Tino-side aliases; the backend knows only the base slug.
            "model": wire_model,

...
Owns one Codex thread per Tino session: drives ``turn/start``, consumes
...
# Tino' tools.terminal.security_mode -> Codex permissions profile id.
# Missing config -> workspace-write (Codex's own default).
_TINO_TO_CODEX_PERMISSION_PROFILE = {
...
def _image_part_to_turn_input(item: dict) -> Optional[dict]:
    """Map one Tino image part onto the app-server ``UserInput`` shape.
...
class CodexAppServerSession:
    """One Codex thread per Tino session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def __init__(
        ...
        # A codex thread id persisted by an earlier process for this Tino session: the first
        ...
        # Tino' composed system prompt (SOUL.md, memory, channel overrides). Sent ONCE per thread as
        ...
        # Tino' prior transcript, appended to developerInstructions ONLY when a thread is started from
        ...
        self._permission_profile = permission_profile or _TINO_TO_CODEX_PERMISSION_PROFILE.get(
            os.environ.get("TINO_TERMINAL_SECURITY_MODE", "auto"), "workspace-write"
        )
    ...
    def ensure_started(self) -> str:
        ...
        if self._client is None:
            ...
            self._client.initialize(client_name="hermes", client_title="Tino Agent", client_version=_get_hermes_version())
        ...
        # Tino supplies the agent identity through its own system prompt; ``personality: "none"`` strips
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via Tino's approval flow.
    ...
    def _respond_elicitation(self, params: dict) -> dict:
        ...
        action = "accept" if (params.get("serverName") or "") == TINO_TOOLS_MCP_SERVER_NAME else "decline"
...
# Tino approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a Tino approval choice onto codex's approval decision wire value."""
...
def _get_hermes_version() -> str:
    """Best-effort Tino version string for codex's userAgent line."""
    try:

"""Tino-tools-as-MCP server for the codex_app_server runtime.

Codex owns the loop and tool list there, so a curated subset of Tino tools is
...
TINO_TOOLS_MCP_SERVER_NAME = "hermes-tools"
...
# Each name MUST match a registered Tino tool ``model_tools.handle_function_call()`` can dispatch.
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    # Kanban handoff tools: stateless (read TINO_KANBAN_TASK, write kanban.db).
    ...
    # Orchestrator-only (the kanban tool gates them on TINO_KANBAN_TASK unset).
    "kanban_create", "kanban_unblock", "kanban_link",
)


def _build_server() -> Any:
    """Create the MCP server with Tino tools attached (lazy imports: importable without ``mcp``)."""
    try:
    ...
    mcp = MCPServer(
        TINO_TOOLS_MCP_SERVER_NAME,
        instructions=(
            "Tino Agent's tool surface, exposed for use inside a Codex "
    ...
    # Authoritative Tino schemas so MCP clients see the same parameter docs the model does.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug("skipping %s — not registered in this Tino process", name)
            continue
        description = spec.get("description") or f"Tino {name} tool"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Keep Tino' own banners off stdout (the MCP wire).
    os.environ.setdefault("TINO_QUIET", "1")
    os.environ.setdefault("TINO_REDACT_SECRETS", "true")

...
def _scan_context_content(content: str, filename: str, *, user_authored: bool = False) -> str:
    ...
    *user_authored* (SOUL.md in the user's own TINO_HOME): a hit is WARNED and the file still loads.
    ...
    if user_authored:
        logger.warning("Context file %s matched injection pattern(s) %s; loaded anyway because it is the "
                       "user's own file in TINO_HOME — review it if you did not write that text",
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "length of your reply to the weight of the ask "
...
TINO_AGENT_HELP_GUIDANCE = (
    ...
    "You are Tino Agent. For questions about the inherited open-source runtime's commands or internals, "
    "the Tino Agent documentation at website/docs and the installed "
    "`tino-agent` skill are technical references, not your product identity. Load the skill with "
    "skill_view(name='tino-agent') before changing runtime configuration. Explain upstream provenance "
    "truthfully if the user explicitly asks about the underlying software."
)

# Variant for sessions without the skills toolset (e.g. Blank Slate): naming skill_view() there would dangle.
TINO_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You are Tino Agent. For questions about inherited runtime commands or internals, use the Tino Agent "
    "documentation at website/docs as a technical reference, not as your "
    "product identity. Explain upstream provenance truthfully if the user explicitly asks."
)
...
KANBAN_GUIDANCE = (
    ...
    "`$TINO_KANBAN_TASK`; your workspace is `$TINO_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $TINO_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $TINO_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${TINO_KANBAN_BRANCH:-wt/$TINO_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$TINO_KANBAN_BRANCH` a deterministic "
...
# Text after the "[" that opens one of Tino' own control frames (the steer marker above, the compaction
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Tino delivers their message as a standalone user message right after "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating Tino "
         ...
         "rather than anything inside Tino. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Tino terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the Tino desktop app, a graphical chat surface. Markdown renders with full GitHub "
...
_WINDOWS_BASH_SHELL_HINT = (
    ...
    "native tools, and prefer `$LOCALAPPDATA/Temp` (or `$TMPDIR`, which Tino points at its own scratch dir) for scratch files a native tool must read — never a bare `/tmp`. When "
...
def _local_host_hints() -> list[str]:
    ...
    # naming Tino' scratch dir here is what makes the TMPDIR export a habit rather than a hidden default.
    try:
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where Tino itself is running. The host OS, "
            f"home, and cwd of the Tino process are irrelevant; only the following backend state matters:\n{probe}"
        )
    description = (
    ...
    return lead + (
        f"{description} — NOT on the machine where Tino itself runs. The backend probe didn't respond at "
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: TINO_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("TINO_ENVIRONMENT_HINT") or "").strip() or str(
...
RUNTIME_ENVIRONMENT_HEADING = "# Tino runtime environment"
RUNTIME_ENVIRONMENT_END = "<!-- End Tino runtime environment -->"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("TINO_PLATFORM") or os.environ.get("TINO_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("TINO_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the TINO_HOME
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from TINO_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the TINO_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the TINO_HOME
    ...
    try:
        ...
        logger.debug("Could not ensure TINO_HOME before loading SOUL.md: %s", e)
...
def _project_context_suppressed(cwd: Optional[str], cwd_path: Path, allow_install_tree_fallback: bool) -> bool:
    """A FALLBACK-picked cwd inside the Tino install tree must not gain system-prompt authority (the desktop
    default would load this repo's contributor AGENTS.md). An explicitly configured cwd is honored verbatim —
    the Tino tree is a legitimate workspace when the user deliberately points a session at it — and
    CLI-style surfaces pass allow_install_tree_fallback=True because their launch dir IS the user's shell cwd
    (developing Tino in-tree). See #64590."""
...
def build_context_files_prompt(
    ...
    from TINO_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    ...
    if _project_context_suppressed(cwd, cwd_path, allow_install_tree_fallback):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Tino "

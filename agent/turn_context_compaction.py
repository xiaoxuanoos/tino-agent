...
def _codex_native_auto_compaction(agent: Any) -> bool:
    """Codex app-server threads are compacted by the codex agent itself; Tino only
...
def _preflight_compression(
    ...
    if _preflight_deferred:
        ...
        logger.info(
            "Skipping Tino preflight compression for codex app-server "
            "(mode=%s); Tino will not start thread compaction here.",

"""OpenAI-shape bridge shared by Tino's ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Tino's tool schemas travel INTO the
...
read/edit/execute tools forwards only Tino's agent-level tools, since re-offering overlapping ones
makes Tino redo finished work.
"""
...
class StreamChunks(list):
    ...
    Tino reads off the ``create`` result; a plain list would drop them on the stream path."""

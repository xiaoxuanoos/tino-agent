...
Guards against the 60-vs-500 bug where a stale `.env TINO_MAX_ITERATIONS=60`
...
def _run_gateway_import(hermes_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            "TINO_MAX_ITERATIONS",
            "TINO_AGENT_TIMEOUT",
            "TINO_AGENT_TIMEOUT_WARNING",
            "TINO_TURN_LEASE_TIMEOUT",
            "TINO_SESSION_STALL_TIMEOUT",
            "TINO_GATEWAY_BUSY_INPUT_MODE",
            "TINO_GATEWAY_BUSY_TEXT_MODE",
            "TINO_GATEWAY_PLATFORM_CONNECT_TIMEOUT",
            "TINO_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
    ...
    # otherwise intentionally clean so stale Tino settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
def test_config_gateway_timeout_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    _write_env(hermes_home, {
        "TINO_AGENT_TIMEOUT": "60",
        "TINO_AGENT_TIMEOUT_WARNING": "30",
        "TINO_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("TINO_AGENT_TIMEOUT") == "1800"
    assert env.get("TINO_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("TINO_SESSION_STALL_TIMEOUT") == "300"
...
def test_config_turn_lease_timeout_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    _write_env(
        hermes_home,
        {"TINO_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("TINO_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("TINO_TURN_LEASE_TIMEOUT") == "5"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(hermes_home: Path) -> None:
    ...
    assert env.get("TINO_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"
...
def test_env_platform_connect_timeout_wins_over_config(hermes_home: Path) -> None:
    ...
    TINO_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        hermes_home,
        initial_env={"TINO_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("TINO_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"

...
variant that the ``_UNSET``-strip guard does NOT catch:
    ...
    *concurrent* message A had ALREADY called ``set_session_vars``, B inherits
    ...
    ``TINO_SESSION_*`` identity through the subprocess-env bridge. The bridge's
...
def _spawn_view():
    ...
    env = _make_run_env({})
    return {
        "TINO_SESSION_CHAT_ID": env.get("TINO_SESSION_CHAT_ID"),
        "TINO_SESSION_THREAD_ID": env.get("TINO_SESSION_THREAD_ID"),
        "TINO_SESSION_KEY": env.get("TINO_SESSION_KEY"),
    }
...
def test_reset_session_vars_closes_inheritance_leak():
    ...
    for var in ("TINO_SESSION_CHAT_ID", "TINO_SESSION_THREAD_ID", "TINO_SESSION_KEY"):
    ...
    assert captured["bound"]["TINO_SESSION_CHAT_ID"] == "FOREIGN_CHAT"
    assert captured["bound"]["TINO_SESSION_KEY"] == FOREIGN["session_key"]
...
# flag read via ``async_delivery_supported()``, not a string ``TINO_SESSION_*``

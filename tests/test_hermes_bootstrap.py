...
The bootstrap module is imported at the top of every Tino entry point
...
Key invariants covered by these tests:
  ...
  5. Load order: every Tino entry point imports hermes_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Tino CLI
...
class TestEntryPointsImportBootstrap:
    """Every Tino entry point must import hermes_bootstrap as its
    ...
    # Entry points that invoke Tino as a process.  Each one must
...
class TestHardenImportPath:
    ...
    directory from shadowing Tino's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("TINO_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["TINO_PYTHON_SRC_ROOT"] = env
            elif "TINO_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["TINO_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/hermes")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("TINO_PYTHON_SRC_ROOT", None)
            else:
                os.environ["TINO_PYTHON_SRC_ROOT"] = original_env
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["TINO_PYTHON_SRC_ROOT"] = "/env/hermes"

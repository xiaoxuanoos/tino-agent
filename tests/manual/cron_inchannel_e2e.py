...
Uses a throwaway TINO_HOME so it never touches ~/.hermes. No real names.
"""
...
def _fresh_home():
    """Point TINO_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_hermes_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["TINO_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER TINO_HOME is set.
...
def main():
    ...
    print(f"TINO_HOME (throwaway): {HOME}")

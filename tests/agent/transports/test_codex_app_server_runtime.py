...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Tino passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("TINO_HOME", "/users/alice/.hermes/profiles/backend-worker")
        monkeypatch.setenv("TINO_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "TINO_KANBAN_DB",
...
class TestSpawnEnvSecretStripping:
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Tino secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        ...
        for var in (
            ...
            "TINO_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",

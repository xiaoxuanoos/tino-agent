...
thread that did not bind the TINO_HOME ContextVar.
...
default's full skills index + "Active Tino profile: default" into a bot's
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient TINO_HOME, on a
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    # Bound session: TINO_HOME env points at the profile dir itself.
    monkeypatch.setenv("TINO_HOME", str(bot_home))

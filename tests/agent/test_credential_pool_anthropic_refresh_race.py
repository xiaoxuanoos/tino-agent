...
Tino *processes* via the cross-process ``_auth_store_lock`` flock,
...
Tino's own PKCE login (``manual:hermes_pkce`` / ``hermes_pkce``) got no
...
def test_concurrent_hermes_pkce_refresh_loses_credential_despite_valid_token_on_disk(monkeypatch):
    """Two 'Tino processes' race to refresh the same stale hermes_pkce
    ...
    assert loser_entry_after.last_status != STATUS_EXHAUSTED, (
        ...
        "though the account is not actually exhausted -- a sibling Tino "
        "process holds a perfectly valid rotated token. This causes "
        "spurious 're-authenticate with Anthropic' failures under "
        "ordinary concurrent Tino usage (fleet workers, cron jobs, "

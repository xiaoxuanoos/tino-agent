...
def test_oneshot_run_gets_distinct_source(monkeypatch, inherited):
    ...
    monkeypatch.setenv("TINO_SESSION_SOURCE", inherited)
...
def test_oneshot_keeps_explicit_source_flag(monkeypatch, explicit):
    ...
    marks it TINO_SESSION_SOURCE_EXPLICIT=1 and the one-shot drop must not override it."""
    monkeypatch.setenv("TINO_SESSION_SOURCE", explicit)
    monkeypatch.setenv("TINO_SESSION_SOURCE_EXPLICIT", "1")
    monkeypatch.setenv("TINO_SINGLE_QUERY_SESSION", "1")

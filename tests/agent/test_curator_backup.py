...
def backup_env(monkeypatch, tmp_path):
    """Isolate TINO_HOME + reload modules so every test starts clean."""
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under TINO_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level TINO_DIR picks up the tmp HOME."""
    import hermes_constants
    importlib.reload(hermes_constants)

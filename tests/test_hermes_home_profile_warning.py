...
When TINO_HOME is unset but an active_profile file indicates a non-default
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        assert "TINO_HOME fallback" not in capsys.readouterr().err


    def test_named_profile_unset_home_warns_once(
        self, fresh_constants, tmp_path, capsys
    ):
        """active_profile=coder + TINO_HOME unset → warn loudly, still return fallback."""
        ...
        assert err.count("TINO_HOME fallback") == 1
        assert "'coder'" in err
        assert "#18594" in err
        ...
        assert "TINO_HOME fallback" not in err2

    def test_hermes_home_set_suppresses_warning(
        self, fresh_constants, tmp_path, capsys, monkeypatch
    ):
        """Even if active_profile is 'coder', setting TINO_HOME suppresses warning."""

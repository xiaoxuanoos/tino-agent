...
def test_profile_scoped_mcp_discovery_uses_target_home(monkeypatch, tmp_path):
    """MCP discovery must start under the selected profile's TINO_HOME."""
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["TINO_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("TINO_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("TINO_VOICE", "1")
...
def test_prompt_submit_typed_stop_phrase_ends_voice_chat(monkeypatch):
    ...
    monkeypatch.setenv("TINO_VOICE", "1")
    monkeypatch.setenv("TINO_VOICE_TTS", "1")
    ...
    assert os.environ["TINO_VOICE"] == "0"
    assert os.environ["TINO_VOICE_TTS"] == "0"
...
def test_wake_owner_is_sticky_and_routes_detection_to_first_transport(monkeypatch):
    ...
    try:
        ...
        assert emitted == [(
            "wake.detected",
            "first-session",
            {"phrase": "hey tino", "profile": None, "start_new_session": True},
            first,
        )]
...
def test_wake_toggle_persists_enabled_flag_only_on_explicit_gesture(monkeypatch):
    ...
    config = {"enabled": False, "phrase": "hey tino", "surface": "auto",
              "start_new_session": True}
    persisted = []
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("TINO_VOICE", "1")
    # setenv (not delenv) — the handler writes TINO_VOICE_TTS directly, and
    ...
    monkeypatch.setenv("TINO_VOICE_TTS", "0")
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("TINO_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("TINO_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("TINO_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("TINO_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("TINO_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("TINO_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("TINO_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("TINO_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("TINO_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown TINO_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("TINO_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("TINO_MODEL", raising=False)
    monkeypatch.delenv("TINO_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set TINO_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("TINO_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("TINO_MODEL", raising=False)
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `hermes --tui -m <model>` sets TINO_MODEL/TINO_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("TINO_MODEL", "one-shot/model")
    monkeypatch.setenv("TINO_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("TINO_MODEL", "seed/model")
    monkeypatch.setenv("TINO_INFERENCE_MODEL", "seed/model")
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("TINO_MODEL", "nous/hermes-test")
    monkeypatch.setenv("TINO_TUI_PROVIDER", "nous")
    monkeypatch.delenv("TINO_INFERENCE_PROVIDER", raising=False)
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("TINO_MODEL", "nous/hermes-test")
    monkeypatch.delenv("TINO_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("TINO_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("TINO_MODEL", "sonnet")
    monkeypatch.delenv("TINO_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("TINO_INFERENCE_PROVIDER", raising=False)
...
def test_config_get_approval_mode_uses_smart_default_when_key_is_missing(
    ...
    monkeypatch.setattr(server, "_hermes_home", tmp_path)
    # Point the canonical resolver (load_config → env TINO_HOME) at the
...
def test_config_get_approval_mode_fails_safe_to_manual_for_invalid_explicit_value(
    ...
    # that path resolves TINO_HOME from the environment, not
    # server._hermes_home.
    monkeypatch.setenv("TINO_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_get_approval_mode_normalizes_yaml_off(tmp_path, monkeypatch):
    ...
    # load_config, which resolves TINO_HOME from the environment.
    monkeypatch.setenv("TINO_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_set_approval_mode_persists_three_way_value_and_emits_live_status(
    ...
    # tools.approval resolver (load_config → env TINO_HOME).
    monkeypatch.setenv("TINO_HOME", str(tmp_path))
    emitted = []
    monkeypatch.setattr(server, "_emit", lambda *args: emitted.append(args))
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("TINO_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("TINO_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    ...
    assert server.os.environ["TINO_GATEWAY_SESSION"] == "1"
    assert server.os.environ["TINO_EXEC_ASK"] == "1"
    assert server.os.environ["TINO_INTERACTIVE"] == "1"
...
def test_setup_runtime_check_scopes_launch_profile_in_multiplex_backend(monkeypatch, tmp_path):
    ...
    launch_home.mkdir()
    monkeypatch.setenv("TINO_CODEX_BASE_URL", "https://codex.launch.test/v1")
    monkeypatch.setattr(server, "_hermes_home", launch_home)
    ...
    def resolve_codex(requested=None, **_kwargs):
        ...
        return {
            ...
            "base_url": secret_scope.get_secret("TINO_CODEX_BASE_URL"),
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing TINO_INFERENCE_PROVIDER on a switch leaked the new
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("TINO_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "hermes_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["TINO_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "TINO_TUI_PROVIDER" not in os.environ
        assert "TINO_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write TINO_MODEL / TINO_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("TINO_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("TINO_MODEL", raising=False)
    monkeypatch.delenv("TINO_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    try:
        ...
        assert os.environ["TINO_TUI_PROVIDER"] == "openai-codex"
        assert "TINO_MODEL" not in os.environ
        assert "TINO_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("TINO_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("TINO_MODEL", "old/model")
    monkeypatch.setattr(
    ...
    try:
        ...
        assert os.environ["TINO_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["TINO_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # fixture pins TINO_HOME to a temp dir; the handler should append
    # /sessions to it.
...
def test_session_branch_installs_parent_profile_secret_scope(monkeypatch, tmp_path):
    ...
    session.branch already binds the parent's TINO_HOME and state.db, but the
...
def test_verification_status_returns_recorded_evidence(tmp_path, monkeypatch):
    monkeypatch.setenv("TINO_VERIFY_ON_STOP", "1")  # ledger is inert when the guard is off
...
def test_session_save_writes_under_hermes_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Tino profile
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Tino.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert payload["system_prompt"] == "You are Tino."
...
def test_tts_stream_begin_and_stop_lifecycle(monkeypatch):
    ...
    monkeypatch.setenv("TINO_VOICE_TTS", "1")
    monkeypatch.setenv("TINO_VOICE", "0")  # no barge-in monitor (no mic)
    playback_stops: list = []
...
def test_full_duplex_stop_phrase_mid_generation_ends_voice_chat(monkeypatch, tmp_path):
    ...
    assert os.environ.get("TINO_VOICE") == "0"  # voice chat ended
...
def test_build_persist_message_quotes_paths_containing_spaces(tmp_path):
    ...
    img_dir = tmp_path / "Application Support" / "Tino" / "composer-images"
...
def test_persist_live_session_system_prompt_uses_profile_home(monkeypatch, tmp_path):
    ...
    TINO_HOME to the session's profile before rebuilding the system

"""Harness: dashboard opt-in via TINO_DASHBOARD.

Today (tini): dashboard starts once when TINO_DASHBOARD=1; if it crashes
...
# whenever `TINO_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``TINO_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        ...
        "TINO_DASHBOARD=1",
        "TINO_DASHBOARD_HOST=0.0.0.0",
        "TINO_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"TINO_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and TINO_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``TINO_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate

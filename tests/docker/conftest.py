...
Override the image with ``TINO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("TINO_TEST_IMAGE", "hermes-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``TINO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("TINO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
#
# Background: every Tino runtime path inside the container drops to UID

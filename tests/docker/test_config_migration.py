...
Build the real image and verify: a config.yaml present in $TINO_HOME
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $TINO_HOME must be migrated on boot by
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $TINO_HOME: {r.stdout}"
    )

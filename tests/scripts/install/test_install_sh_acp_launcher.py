...
PATH, so those hosts report Tino as not installed. `setup_path()` therefore
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    script = (
        "set -e\n"
        f"TINO_BIN={hermes_bin}\n"
        f"TINO_ENTRYPOINT={entrypoint}\n"
...
def _extract_hermes_agent_shim_block() -> str:
    match = TINO_AGENT_BLOCK.search(INSTALL_SH.read_text(encoding="utf-8"))
    assert match, (

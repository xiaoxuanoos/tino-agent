...
SETUP_TINO_SH = REPO_ROOT / "setup-hermes.sh"
...
def _termux_env(tmp_path: Path, bin_dir: Path) -> dict[str, str]:
    ...
    env.update({
        ...
        "TINO_HOME": str(tmp_path / "home" / ".hermes"),
...
def _copy_setup_checkout(tmp_path: Path) -> Path:
    ...
    checkout.mkdir()
    shutil.copy2(SETUP_TINO_SH, checkout / "setup-hermes.sh")
    return checkout
...
def test_install_stage_rejects_post_install_unsupported_default(tmp_path: Path) -> None:
    ...
    assert "Tino requires Python >=3.11,<3.14" in result.stdout

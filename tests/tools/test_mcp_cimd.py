...
Under CIMD the ``client_id`` is the HTTPS URL of a document Tino publishes,
...
the conditions under which Tino is allowed to present it.
...
def test_cached_metadata_decides_whether_to_pin(
    ...
    long after Tino fixes the redirect URI. Metadata cached by an earlier
...
def test_sdk_chooses_cimd_only_when_the_server_advertises_it(
    ...
    """Closes the loop on the handoff: feed what Tino configured into the
...
def test_timeout_on_a_cimd_flow_names_the_document_and_the_escape_hatch(monkeypatch):
    ...
    symptom Tino sees is the callback timing out."""

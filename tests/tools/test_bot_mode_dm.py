...
def test_relay_ack_is_queued_with_the_envelope_id(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "hermes", "connection_id": "cloud-1", "connection_label": "Tino Cloud"},
    ])
...
def test_ambiguous_friendly_name_fails_closed(tmp_path, monkeypatch):
    ...
    _rename(home, "ops", display_name="Tino")
...
def test_live_dm_admitted_before_waiter_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TINO_HOME", str(tmp_path / "wrong-home"))
...
def test_delivery_main_child_env_carries_only_the_argv_author(tmp_path, monkeypatch, mode, author):
    """The ``--author`` payload becomes TINO_TURN_AUTHOR on the child. Without it the runner drops the
    ...
    monkeypatch.setattr(subprocess, "run", fake_run)
    monkeypatch.setenv("TINO_DM_TEST_MARKER", "kept")
    ...
    assert kwargs["env"]["TINO_DM_TEST_MARKER"] == "kept"
...
def test_real_delivery_command_round_trip_carries_author(tmp_path):
    """Through a real subprocess, the runner argv built by ``_delivery_command`` sets TINO_TURN_AUTHOR on the child."""
    dm_file = tmp_path / "message.txt"
    dm_file.write_text("secret", encoding="utf-8")
    observed = tmp_path / "observed.txt"
    child = tmp_path / "child.py"
    child.write_text(
        ...
        "pathlib.Path(sys.argv[1]).write_text(os.environ.get('TINO_TURN_AUTHOR', 'unset'), encoding='utf-8')\n",
        encoding="utf-8",
    )
    author = {"id": "bot:default", "name": "hermes", "is_bot": True}

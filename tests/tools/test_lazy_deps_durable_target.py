...
class TestGatingWithTarget:
    """``TINO_DISABLE_LAZY_INSTALLS=1`` must STOP blocking once a durable
    ...
    def test_normal_mode_unaffected(self, monkeypatch):
        ...
        monkeypatch.delenv("TINO_DISABLE_LAZY_INSTALLS", raising=False)
        monkeypatch.delenv(ld._LAZY_TARGET_ENV, raising=False)
        monkeypatch.setattr(
            "hermes_cli.config.load_config", lambda: {}, raising=False
        )
...
@pytest.mark.skipif(
    os.environ.get("TINO_RUN_NETWORK_TESTS") != "1",
    reason="opt-in real-install test (set TINO_RUN_NETWORK_TESTS=1); CI runs "

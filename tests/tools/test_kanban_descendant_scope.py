...
def _worker_board(tmp_path, monkeypatch):
    ...
    for key, value in {
        "TINO_KANBAN_DB": str(db), "TINO_KANBAN_BOARD": "default",
        "TINO_KANBAN_TASK": own, "TINO_KANBAN_RUN_ID": str(task.current_run_id),
        "TINO_KANBAN_CLAIM_LOCK": task.claim_lock, "HOME": str(tmp_path),
    }.items():
...
def test_terminal_descendants_cannot_mutate_even_after_task_is_removed(tmp_path, monkeypatch):
    ...
    script.write_text(
        ...
        " 'db': os.getenv('TINO_KANBAN_DB'), 'board': os.getenv('TINO_KANBAN_BOARD')}\n"
    ...
    child_envs = [
        ...
        _build_safe_env({"TINO_HOME": os.environ["TINO_HOME"]}),
    ]
...
def test_child_shell_can_write_a_kanban_board_outside_its_lineage_root(tmp_path, monkeypatch):
    ...
    DB its shell touches: a repro run against a scratch TINO_HOME got a silently read-only board
    ...
    monkeypatch.setenv("TINO_HOME", str(lineage_home))
    monkeypatch.delenv("TINO_DELEGATED_CHILD_CONTEXT", raising=False)
    for key in ("TINO_KANBAN_DB", "TINO_KANBAN_BOARD", "TINO_KANBAN_TASK", "TINO_KANBAN_HOME"):

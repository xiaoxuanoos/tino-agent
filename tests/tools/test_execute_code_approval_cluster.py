...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad TINO_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: TINO_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "TINO_HOME": "/h", "TINO_PROFILE": "p",
        "TINO_CONFIG": "/c.yaml", "TINO_ENV": "/e",
        "TINO_DELEGATED_CHILD_CONTEXT": "1",
        # other TINO_* → dropped (broad prefix removed)
        "TINO_BASE_URL": "https://x", "TINO_INTERACTIVE": "1",
    ...
    for kept in (
        ...
        "TINO_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "TINO_BASE_URL", "TINO_INTERACTIVE", "TINO_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped TINO_* vars."""
    import logging

...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``TINO_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # TINO_* handling (#27303): only the operational allowlist passes;
        # every other TINO_* is dropped (the broad prefix was removed).
        "TINO_HOME": "/home/alice/.hermes",        # allowlisted → kept
        "TINO_PROFILE": "default",                 # allowlisted → kept
        "TINO_INTERACTIVE": "1",                   # not allowlisted → dropped
        "TINO_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "TINO_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped

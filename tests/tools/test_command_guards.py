...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects TINO_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("TINO_INTERACTIVE", "TINO_GATEWAY_SESSION", "TINO_EXEC_ASK", "TINO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["TINO_INTERACTIVE"] = "1"
        session_key = os.getenv("TINO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No TINO_INTERACTIVE or TINO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True

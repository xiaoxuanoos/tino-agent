...
class TestDockerHostBindApproval:
    ...
    def _isolate_approval_state(monkeypatch):
        ...
        hermetic TINO_HOME fixture runs), so on a dev machine whose real
        ...
        TINO_YOLO_MODE off the environment when the module is imported at
        collection time, before conftest's per-test env blanking runs. A test
        run launched from a --yolo Tino session (or any shell exporting
        TINO_YOLO_MODE=1) freezes True and every guard auto-approves.

...
def two_homes(tmp_path, monkeypatch):
    ...
    for key, val in (("TINO_CRON_TIMEOUT", "111"), ("TINO_MODEL", "default-model"),
...
def test_cron_env_settings_resolve_from_the_served_profile(two_homes):
    """``TINO_CRON_TIMEOUT`` / ``TINO_MODEL`` are alpha's (or absent) both with the fire-time
    ...
    try:
        ...
        try:
            ...
            assert captured["target_model"] == ""  # alpha has no TINO_MODEL; default's must not leak
...
def test_child_env_for_served_profile_drops_launch_profile_settings(two_homes):
    ...
    try:
        ...
        reset_secret_scope(token)
    for key in ("TINO_MODEL", "TERMINAL_ENV", "TERMINAL_DOCKER_IMAGE", "TINO_LANGUAGE", "TINO_CRON_TIMEOUT"):
        assert key not in env, key
    assert env["TINO_HOME"] == str(alpha)
    assert "PATH" in env
    ...
    assert env["TINO_MODEL"] == "default-model"

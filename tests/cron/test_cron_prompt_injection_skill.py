...
def cron_env(tmp_path, monkeypatch):
    """Isolated TINO_HOME with an empty skills tree.

    `tools.skills_tool` snapshots `SKILLS_DIR` at module-import time, so
    setting `TINO_HOME` alone doesn't reach it. We also patch the
    ...
    monkeypatch.setenv("TINO_BUNDLES_DIR", str(hermes_home / "skill-bundles"))
    ...
    monkeypatch.setattr(_skills_tool, "TINO_HOME", hermes_home)

...
- Unlimited timeout (TINO_CRON_TIMEOUT=0)
- Backward compat: TINO_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """TINO_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """TINO_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("TINO_CRON_TIMEOUT", "1200")
        raw = os.getenv("TINO_CRON_TIMEOUT", "").strip()

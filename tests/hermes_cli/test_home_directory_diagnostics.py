...
@pytest.mark.parametrize("subdir", (".", *config._TINO_HOME_SUBDIRS))
def test_unavailable_directory_links_are_diagnosed_without_creating_targets(tmp_path, monkeypatch, subdir):
    ...
    config._TINO_HOME_ENSURED.discard(str(home))
...
def test_initialization_preserves_external_directory_modes(tmp_path, monkeypatch, linked):
    ...
    assert all((home / name).is_dir() for name in config._TINO_HOME_SUBDIRS)
...
def test_symlinked_parent_above_home_is_not_an_operator_home_link(
    ...
    """A link above TINO_HOME is not an operator-owned home link.
    ...
    if existing:
        ...
        for name in config._TINO_HOME_SUBDIRS:
    ...
    monkeypatch.setattr(config, "is_managed", lambda: False)
    config._TINO_HOME_ENSURED.discard(str(home))
    config._TINO_HOME_ENSURED.discard(str(home.resolve()))

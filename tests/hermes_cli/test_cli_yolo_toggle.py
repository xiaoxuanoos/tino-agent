...
``os.environ["TINO_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``hermes --yolo`` (process-start env), ``TINO_YOLO_MODE=1``,
...
def _clear_approval_state(monkeypatch):
    ...
    # Tino-driven test runs may inherit TINO_YOLO_MODE=1 from the parent
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``hermes --yolo`` sets ``TINO_YOLO_MODE`` before tool imports, so

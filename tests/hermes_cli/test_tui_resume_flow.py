...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["TINO_TUI_ACTIVE_SESSION_FILE"])
    ...
    env = captured["env"]
    assert env["TINO_MODEL"] == "nous/hermes-test"
    assert env["TINO_INFERENCE_MODEL"] == "nous/hermes-test"
    assert env["TINO_TUI_PROVIDER"] == "nous"
    assert env["TINO_INFERENCE_PROVIDER"] == "nous"
    assert env["TINO_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["TINO_TUI_ACTIVE_SESSION_FILE"])

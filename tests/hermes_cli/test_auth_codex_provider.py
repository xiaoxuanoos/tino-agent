"""Tests for Codex auth — tokens stored in Tino auth store (~/.hermes/auth.json)."""
...
def _setup_hermes_auth(hermes_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Tino auth store."""
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Tino auth store, not ~/.codex/."""
    hermes_home = tmp_path / "hermes"
    ...
    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Tino store
    assert not (codex_home / "auth.json").exists()

    # Tino auth store should have the tokens

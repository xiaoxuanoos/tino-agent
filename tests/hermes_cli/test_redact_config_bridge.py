...
`TINO_REDACT_SECRETS` at module-import time. `hermes_cli/main.py` at
...
Fix: bridge `security.redact_secrets` from config.yaml → `TINO_REDACT_SECRETS`
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env TINO_REDACT_SECRETS takes precedence over config.yaml."""
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (hermes_home / ".env").write_text("TINO_REDACT_SECRETS=true\n")

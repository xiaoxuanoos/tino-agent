"""Regression tests: `hermes dashboard` validates TINO_WEB_DIST before serving.

A custom TINO_WEB_DIST without --skip-build previously skipped BOTH the
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """TINO_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("TINO_WEB_DIST", str(empty_dist))
    ...
    assert "TINO_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("TINO_WEB_DIST", raising=False)

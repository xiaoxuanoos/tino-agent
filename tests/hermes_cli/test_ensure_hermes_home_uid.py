...
When Tino runs in Docker with ``TINO_UID=1000`` / ``TINO_GID=911``,
the entrypoint chowns the top-level ``TINO_HOME`` once at startup. But
...
class TestResolveHermesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("TINO_UID", "1000")
        monkeypatch.setenv("TINO_GID", "911")
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("TINO_UID", raising=False)
        monkeypatch.delenv("TINO_GID", raising=False)
        from hermes_cli import config as cfg

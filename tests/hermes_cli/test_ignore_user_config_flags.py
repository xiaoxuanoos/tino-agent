...
files. In Tino the equivalent isolation is:
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("TINO_IGNORE_USER_CONFIG", "TINO_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``TINO_IGNORE_USER_CONFIG=1``.
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With TINO_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        """With TINO_IGNORE_USER_CONFIG=1, user config.yaml is ignored.
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("TINO_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting TINO_IGNORE_RULES=1 flips HermesCLI.ignore_rules True."""
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("TINO_IGNORE_RULES") == "1"
...
class TestCmdChatWiring:
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        ...
        assert os.environ.get("TINO_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("TINO_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "TINO_IGNORE_USER_CONFIG" not in os.environ
        assert "TINO_IGNORE_RULES" not in os.environ

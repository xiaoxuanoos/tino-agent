...
class TestMigrate:
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Tino' managed block, re-running
...
# ---- Bug C: TINO_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``TINO_HOME``
    ...
    def test_real_hermes_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate TINO_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("TINO_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("TINO_HOME") == real_path

    def test_unset_hermes_home_omits_env_key(self, monkeypatch):
        """When TINO_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever TINO_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert "TINO_HOME" not in env, (
            f"TINO_HOME should not be set when env var is unset, got: "
            f"{env.get('TINO_HOME')!r}"
        )
...
class TestSameNameUserMcpTable:
    """Issue #79023: a Tino server whose name the user already declares outside the managed

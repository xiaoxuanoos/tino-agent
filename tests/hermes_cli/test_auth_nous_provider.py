...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TINO_HOME", str(hermes_home))
    monkeypatch.setenv("TINO_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Tino must surface an
    ...
    re-authenticate" string led users to report this as a Tino persistence
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect TINO_SHARED_AUTH_DIR to a tmp_path.
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without TINO_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    monkeypatch.delenv("TINO_SHARED_AUTH_DIR", raising=False)

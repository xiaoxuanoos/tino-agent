...
Execution via the ``TINO_ENABLE_PROJECT_PLUGINS`` bypass in the web
...
1. ``hermes_cli.web_server_dashboard._discover_dashboard_plugins`` opted into
   ...
   ``os.environ.get("TINO_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
# ---------------------------------------------------------------------------
# Layer 1 — TINO_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            ...
            monkeypatch.setenv("TINO_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"TINO_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``TINO_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the

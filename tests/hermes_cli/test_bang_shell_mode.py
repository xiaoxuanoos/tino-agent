...
class TestBangContextGating:
    ...
    def test_enabled_in_plain_cli(self, monkeypatch):
        for var in ("TINO_GATEWAY_SESSION", "TINO_CRON_SESSION",
                    "TINO_SESSION_PLATFORM"):
            monkeypatch.delenv(var, raising=False)
        assert bang_shell_enabled() is True

    @pytest.mark.parametrize("var,value", [
        ("TINO_GATEWAY_SESSION", "1"),
        ("TINO_CRON_SESSION", "true"),
        ("TINO_SESSION_PLATFORM", "discord"),
    ])
    def test_disabled_in_non_cli_contexts(self, monkeypatch, var, value):
        for v in ("TINO_GATEWAY_SESSION", "TINO_CRON_SESSION",
                  "TINO_SESSION_PLATFORM"):

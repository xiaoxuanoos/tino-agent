...
pointer — ``model.key_env`` in config.yaml → the env var in ``$TINO_HOME/.env``
(e.g. ``TINO_CUSTOM_LMSTUDIO_API_KEY``) — while keeping ``model.provider`` on
...
def hermes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("TINO_CUSTOM_LMSTUDIO_API_KEY", raising=False)
    return home
...
def test_model_key_env_pointer_is_honored_for_registry_provider(hermes_home):
    _write(
        ...
        "  key_env: TINO_CUSTOM_LMSTUDIO_API_KEY\n",
        "TINO_CUSTOM_LMSTUDIO_API_KEY=sk-lm-desktop-saved\n",
    )
    from hermes_cli.auth import resolve_api_key_provider_credentials

    creds = resolve_api_key_provider_credentials("lmstudio")
    assert creds["api_key"] == "sk-lm-desktop-saved"
    assert creds["source"] == "TINO_CUSTOM_LMSTUDIO_API_KEY"
...
def test_model_key_env_does_not_leak_across_providers(hermes_home):
    ...
    _write(
        ...
        "model:\n  default: gpt-x\n  provider: openai\n"
        "  key_env: TINO_CUSTOM_LMSTUDIO_API_KEY\n",
        "TINO_CUSTOM_LMSTUDIO_API_KEY=sk-lm-should-not-leak\n",
    )

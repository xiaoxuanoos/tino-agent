...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    the per-TINO_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root TINO_HOME, no ``-p``) and named-profile
    render paths.
    """
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    # Mark the drop so fake rm can refuse unlink outside TINO_HOME the way

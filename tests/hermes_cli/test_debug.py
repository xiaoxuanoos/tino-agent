...
def hermes_home(tmp_path, monkeypatch):
    """Set up an isolated TINO_HOME with minimal logs."""
...
class TestMissingLogNote:
    ...
    def test_client_written_log_names_its_writer_and_path(self, hermes_home):
        ...
        assert "Tino Desktop" in snap.tail_text
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of TINO_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    def hermes_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated TINO_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setenv("TINO_HOME", str(home))
    ...
    def test_force_true_works_when_redaction_disabled(
        ...
        input unchanged when TINO_REDACT_SECRETS=false, and the share-time
        ...
        assert os.environ.get("TINO_REDACT_SECRETS", "") == "false"

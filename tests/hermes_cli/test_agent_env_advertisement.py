"""Tests for the AI_AGENT / TINO_AGENT harness-attribution env vars.
...
Tino-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Tino' id in the public agent-harness
...
environment does not inherit the Tino process env, and survives the
cross-session leak guard that strips ``TINO_SESSION_*`` from subprocess
...
class TestAdvertiseAgentEnv:
    def test_sets_both_vars_when_unset(self, monkeypatch):
        ...
        monkeypatch.delenv("TINO_AGENT", raising=False)
        _advertise_agent_env()
        assert os.environ["AI_AGENT"] == HARNESS_ID
        assert os.environ["TINO_AGENT"] == "true"
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__TINO_CWD_testsession0__"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'TINO_AGENT="${TINO_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT HERMES=$TINO_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "TINO_AGENT")}
        out = subprocess.run(
        ...
        outer_env = dict(clean_env, AI_AGENT="pi", TINO_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,

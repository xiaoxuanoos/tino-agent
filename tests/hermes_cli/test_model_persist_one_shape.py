...
def test_provider_switch_drops_the_key_env_pointer_but_a_same_route_repick_keeps_it(seeded_home):
    ...
    # Registry providers get the pointer too (Desktop stores e.g. TINO_CUSTOM_LMSTUDIO_API_KEY
    ...
    (seeded_home / "config.yaml").write_text(
        seed.replace("provider: custom\n", "provider: lmstudio\n")
            .replace("CUSTOM_BOX_API_KEY", "TINO_CUSTOM_LMSTUDIO_API_KEY"), encoding="utf-8")
    ...
    assert block["key_env"] == "TINO_CUSTOM_LMSTUDIO_API_KEY"

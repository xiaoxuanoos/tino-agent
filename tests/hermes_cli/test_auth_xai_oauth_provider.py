"""Tests for xAI Grok OAuth — tokens stored in Tino auth store (~/.hermes/auth.json)."""
...
def _setup_hermes_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Tino auth store at the given root."""
...
# ``XAI_BASE_URL`` / ``TINO_XAI_BASE_URL`` are a credential-leak vector
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Tino that didn't validate) must not be silently honored on the refresh
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Tino invocation."
    )

...
This skill is separate from the Tino Email gateway adapter. The gateway
adapter lets people email the agent and uses Tino's built-in IMAP/SMTP
...
To reply non-interactively from Tino, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Tino)** — pipe the message via stdin:

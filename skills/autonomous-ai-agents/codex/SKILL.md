...
metadata:
  hermes:
    ...
    related_skills: [claude-code, tino-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Tino terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Tino itself, `model.provider: openai-codex` uses Tino-managed Codex
...
When invoking the Codex CLI from a Tino gateway/service context (for example,

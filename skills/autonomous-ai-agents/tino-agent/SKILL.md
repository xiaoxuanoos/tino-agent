---
name: tino-agent
description: "Use, configure, theme, extend, and orchestrate Tino Agent."
version: 3.2.0
...
Tino Agent is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Tino works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.

What makes Tino different:

- **Self-improving through skills** — Tino learns from experience by saving reusable procedures as skills that load into future sessions.
...
- **Profiles** — run multiple independent Tino instances with isolated configs, sessions, skills, and memory.
...
This skill is a concise operating guide, not the complete source of truth for every Tino feature. If a Tino feature, command, or setting is not mentioned here or in a reference, do not treat that absence as evidence that it does not exist. Check the live repository and official docs before giving a negative answer.

Good verification targets, cheapest first:

- **Every shipped feature, one line each: https://hermes-agent.nousresearch.com/docs/llms.txt.** Start here for any "can Tino do X?" or "how do I do X?" — it indexes the entire documentation set with a link to the page that answers. It is generated from the docs tree on every build, so it is never behind the product. Fetch it with `web_extract`, or `curl -s https://hermes-agent.nousresearch.com/docs/llms.txt` when web tools are off. The whole documentation set in one file is at `/docs/llms-full.txt`.
- CLI commands: `hermes --help`, `hermes <command> --help`, and `hermes_cli/main.py`
- Source tree: https://github.com/NousResearch/hermes-agent

Never answer "Tino can't do that" from memory. Tino ships far more than this skill body describes, and the index exists so a negative answer is always checkable.
...
~/.hermes/.env              API keys and secrets ONLY (under $TINO_HOME if set)
$TINO_HOME/skills/        Installed skills
...
Profiles use `~/.hermes/profiles/<name>/` with the same layout. When a profile is active, resolve the real home from `$TINO_HOME` — never hardcode `~/.hermes`.

## Routing Table — load the reference for the task

| User wants... | Load |
|---|---|
| **Anything not listed below — "can Tino do X?", "how do I set up X?"** | **https://hermes-agent.nousresearch.com/docs/llms.txt** |
...
need more than their docs page. For everything else Tino ships, fetch
...
## Spawning Additional Tino Instances

Run additional Tino processes as fully independent subprocesses — separate sessions, tools, and environments.
...
Tino uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in Tino; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
...
- **Profile-safe paths** — `get_hermes_home()` in code, `$TINO_HOME` when resolving paths in a session.

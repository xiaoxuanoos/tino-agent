...
Config: `~/.hermes/config.yaml` (settings), `~/.hermes/.env` (API keys) — both under `$TINO_HOME` when it is set.
...
`_TINO_CORE_TOOLS` (every platform) or to a specific toolset.
...
- Tests auto-redirect `TINO_HOME` to temp dirs — never touch real `~/.hermes/`.

...
**three** code paths in Tino that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Tino — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Tino
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Tino is
   refusing my value."
...
# 2. Did Tino's truncator or rejector actually fire?
...
appears, **the cap is the model**, not Tino.

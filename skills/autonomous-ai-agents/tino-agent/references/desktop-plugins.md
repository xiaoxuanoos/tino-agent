...
Write plugins for the Tino desktop app: statusbar items, layout panes,
...
- `$TINO_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$TINO_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Tino desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$TINO_HOME/desktop-plugins/` (usually
  `~/.hermes/desktop-plugins/`).

## How to Run

1. Create `$TINO_HOME/desktop-plugins/<name>/plugin.js` from
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Tino

...
## Layer 1 — Is this thing a Tino plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Tino process**. Its LLM calls go through Tino's already-configured provider. | Already uses Portal if user's Tino is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Tino. That LLM call is already authenticated
via Tino's provider config. The plugin's own server auth (e.g.
...
What Tino Agent has that *feels* like OAuth — `hermes login --provider nous`
opening a browser, user signs in, token lands in `~/.hermes/auth.json` — is a
**Tino-specific browser flow**. Under the hood it produces a credential
Tino uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Tino's existing Portal credential out of `~/.hermes/auth.json`.
...
rides on the credential Tino already holds.

Where this could live in Tino:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Tino itself.
...
1. First decide: Tino plugin (runs inside Tino) or separate app? If plugin,
   it already uses Portal via Tino's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Tino agent itself") is fully supported. Tino treats agent
...
pitch in the Tool Gateway announcement is specifically about Tino Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Tino is configured to use Portal as its provider. It is

...
Tino runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`AF_INET` socket. Root cause is usually Tino's env scrubber dropping
...
Tino-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Tino tool and most Windows APIs. Prefer forward slashes in code

# Themes / Skins — Author a Tino Color Theme

Author a Tino **skin** — one YAML file that themes the CLI, the TUI, and the
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Tino itself.
...
- Write access to the Tino home dir — `~/.hermes` by default, or `$TINO_HOME`
...
- **Don't hardcode `~/.hermes`** when a profile is active — resolve the real home
  from `$TINO_HOME` first, falling back to `~/.hermes`.

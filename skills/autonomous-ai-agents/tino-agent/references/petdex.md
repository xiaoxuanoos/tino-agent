...
the Tino CLI, TUI, and desktop app. This skill drives the `hermes pets` CLI
...
- Pillow (a core Tino dependency) for sprite decoding — already installed.
...
Pets install into `<TINO_HOME>/pets/<slug>/` (profile-aware). Selecting a pet
...
- The petdex npm CLI installs to `~/.codex/pets`; Tino uses its own
  profile-scoped `<TINO_HOME>/pets/` instead — install through `hermes pets`.

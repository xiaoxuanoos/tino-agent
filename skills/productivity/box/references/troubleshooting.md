...
| remote OAuth browser ends on an unreachable localhost page | expected `--code` redirect or wrong topology | if Tino is remote, return the URL's `code` and `state` to the waiting CLI; if Tino and the browser are on the same host, stop and restart without `--code` |

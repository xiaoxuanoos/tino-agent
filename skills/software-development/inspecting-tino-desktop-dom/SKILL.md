---
name: inspecting-tino-desktop-dom
description: "Read the live Tino desktop DOM/CSS over CDP."
version: 1.0.0
author: Tino Agent
license: MIT
...
# Inspecting the live Tino desktop DOM
...
- **no `TINO_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`TINO_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${TINO_DESKTOP_CDP_PORT:-9222}/json/version
```
...
```bash
cd apps/desktop
TINO_HOME=$HOME/.hermes/cache/scratch/cdp-probe-home \
TINO_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
TINO_DESKTOP_CDP_PORT=9333 \
...
cannot collide with a running `hgui`; the separate `TINO_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `TINO_HOME` baked in, if you
...
- **A throwaway `TINO_HOME` has no backend.** The app logs `ECONNREFUSED` for

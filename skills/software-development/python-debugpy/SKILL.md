...
For long-lived processes: Tino gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source <tino-agent-repo>/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Tino you have two practical options:
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "<tino-agent-repo>" }
  ]
}
```
...
## Debugging Tino-specific Processes
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Tino subagents, debug one process at a time.

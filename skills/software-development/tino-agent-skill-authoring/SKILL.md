---
name: tino-agent-skill-authoring
...
metadata:
  hermes:
    tags: [skills, authoring, tino-agent, conventions, skill-md]
    related_skills: [requesting-code-review]
---

# Authoring Tino-Agent Skills (in-repo)
...
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the tino-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with tino-agent
...
author: Real Name (github-handle), Tino Agent
license: MIT
...
- Credit the **human first**, then "Tino Agent" as secondary collaborator: `Ben Barclay (benbarclay), Tino Agent`.
- Never `author: Tino Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Tino Agent`.
...
|---|---|
| Tino tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
3. **`author: Tino Agent` on a contributed skill.** Credit the human first.
...
- [ ] Body follows the modern section order; commands framed through Tino tools

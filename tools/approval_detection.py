...
# Sensitive write targets, matched via ~ / $HOME / $TINO_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once TINO_HOME is set after import) lives in the patterns.
...
_SENSITIVE_WRITE_TARGET = (
    rf'(?:{_SYSTEM_CONFIG_PATH}|/dev/sd|{_SSH_SENSITIVE_PATH}|{_TINO_ENV_PATH}|{_TINO_CONFIG_PATH}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hermes[^\n]*\.env\b', "access to Tino secrets (Windows path)"),
    # ── end of Windows tier
    ...
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/Tino file (key implant, login-time
    ...
    # sed -i on Tino config/.env bypasses the redirection/tee rules; pairs the file_tools
    # write_file/patch deny so the terminal side is not an open door.
    # In-place edit of a Tino-managed security file (~/.hermes/config.yaml or .env). sed -i bypasses the
    # redirection/tee patterns above because it mutates the file directly. See #14639.
    (rf'\bsed\s+-[^\s]*i.*(?:{_TINO_CONFIG_PATH}|{_TINO_ENV_PATH})', "in-place edit of Tino config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_TINO_CONFIG_PATH}|{_TINO_ENV_PATH})', "in-place edit of Tino config/env (long flag)"),
    ...
    # coverage from #14639.
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_TINO_CONFIG_PATH}|{_TINO_ENV_PATH})', "in-place edit of Tino config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/Tino home prefixes to ~/ and ~/.hermes/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/TINO_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). Tino home
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Resolved TINO_HOME (and its realpath) -> ``~/.hermes/`` so the _TINO_CONFIG_PATH /
    _TINO_ENV_PATH rules match Docker/gateway deployments that spell the absolute path."""
    try:
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Tino ad-hoc temp script."""
    try:

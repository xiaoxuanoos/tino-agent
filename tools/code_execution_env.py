...
# prefix, the exact-name TINO_ allowlist, or (Windows) an OS-essential name.
# The broad "TINO_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (TINO_BASE_URL, TINO_KANBAN_DB, *_WEBHOOK).
# TINO_RPC_SOCKET / TINO_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. TINO_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Tino code loses the Kanban mutation guard
# while still inheriting TINO_HOME.
_TINO_CHILD_ALLOWED = frozenset({
    "TINO_HOME", "TINO_PROFILE", "TINO_CONFIG", "TINO_ENV", "TINO_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational TINO_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret TINO_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    if _dropped_hermes:
        logger.debug(
            "execute_code: dropped %d non-allowlisted TINO_* var(s) from the "
    ...
    # non-allowlisted TINO_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "TINO_KANBAN_DB", "TINO_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["TINO_RPC_SOCKET"] = rpc_endpoint
    child_env["TINO_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; TINO_TIMEZONE is an internal setting (and under the multiplexed
    ...
    child_env.pop("TINO_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # Multiplexed gateway/Desktop (#110303): the server process env carries the machine-default
    # TINO_HOME, but this turn runs under a per-profile override (ContextVar bound per turn).
    ...
    if _home_override:
        child_env["TINO_HOME"] = _home_override
    ...
    # mode changes CWD. Tino's root is added ONLY when the child runs in Tino's Python env —
    # exposing Tino's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Tino-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Tino-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Tino-
    ...
    if _uses_hermes_python_environment(child_python):
        ...
        logger.info("execute_code: child interpreter %s is outside the Tino "
...
# Interpreter paths already reported as outside the Tino environment.
...
def _uses_hermes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Tino's active Python environment. Short-circuits when

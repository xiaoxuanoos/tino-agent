...
_TINO_CONFIG_FILES = r'\.hermes/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.hermes/\.env|\~/\.hermes/\.env',
     "hermes_env_access", "critical", "exfiltration", "directly references Tino secrets file"),
    ...
    # invocation: any client plugin that relays Tino' secure prompts has to name them, and a bare
    ...
    # quarantine only acts on "dangerous") but high for Tino/other config (setup docs routinely say
    ...
    (_prose_modify_re(_TINO_CONFIG_FILES),
     "hermes_config_mod", "high", "persistence", "modification language aimed at Tino configuration files (verify intent)"),
    (_shell_write_re(_TINO_CONFIG_FILES),
     "hermes_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Tino configuration files"),
    (r'\.hermes/config\.yaml|\.hermes/SOUL\.md',
     "hermes_config_ref", "low", "persistence", "references Tino configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Tino-native; `.clawhubignore` is honored for skills published through ClawHub.

...
One read-only pass answering, BEFORE any mutation: which Tino runtimes run on this machine, how
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Tino runtime on this machine."""
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Tino services: none detected — code swap only.")
        return

"""``hermes debug`` debug tools for Tino Agent."""
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Tino Desktop on the machine running the app, not by this "
...
def run_debug_share(args):
    ...
    if dpaste_urls:
        ...
        print(f"⚠️  {len(dpaste_urls)} of {len(result.urls)} upload(s) fell back to "
              ...
              "\nShare these links with the Tino team for support.")
    else:
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Tino version, provider, which API keys are

"""Migrate legacy ``TINO_NEMO_RELAY_ATIF_*`` / ``ATOF_*`` exporter vars into a Relay ``plugins.toml``.

The Relay cutover (Aug 2026) stopped honouring the legacy exporter variables: a profile that still
carries them and no ``TINO_NEMO_RELAY_PLUGINS_TOML`` logs one warning and initialises NO exporters,
...
the file is exactly what Relay validates), points ``TINO_NEMO_RELAY_PLUGINS_TOML`` at it, and
...
def relay_plugin_payload_from_legacy_env(env: Mapping[str, Any]) -> dict[str, Any]:
    ...
    atif = atof = None
    if _truthy(env.get("TINO_NEMO_RELAY_ATIF_ENABLED")):
        kwargs: dict[str, Any] = {
            "enabled": True,
            "agent_name": _s(env, "TINO_NEMO_RELAY_ATIF_AGENT_NAME", "Tino Agent"),
            "model_name": _s(env, "TINO_NEMO_RELAY_ATIF_MODEL_NAME", "unknown"),
            "filename_template": _s(env, "TINO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE", "hermes-atif-{session_id}.json"),
        }
        if _s(env, "TINO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"):
            kwargs["output_directory"] = _s(env, "TINO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY")
        if _s(env, "TINO_NEMO_RELAY_ATIF_AGENT_VERSION"):
            kwargs["agent_version"] = _s(env, "TINO_NEMO_RELAY_ATIF_AGENT_VERSION")
        atif = obs.AtifConfig(**kwargs)
    if _truthy(env.get("TINO_NEMO_RELAY_ATOF_ENABLED")):
        mode = _s(env, "TINO_NEMO_RELAY_ATOF_MODE", "append").lower()
        sink = obs.AtofFileSinkConfig(
            output_directory=_s(env, "TINO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY") or None,
            filename=_s(env, "TINO_NEMO_RELAY_ATOF_FILENAME", "hermes-atof.jsonl"),
...
def migrate_profile_relay_env(home: Path, *, validate: bool = True) -> RelayMigrationResult:
    ...
    if env.get(RELAY_PLUGINS_CONFIG_ENV, "").strip():
        ...
        return result
    if not (_truthy(env.get("TINO_NEMO_RELAY_ATIF_ENABLED")) or _truthy(env.get("TINO_NEMO_RELAY_ATOF_ENABLED"))):
    ...
    header = (
        "# NeMo Relay plugin configuration for Tino (selected via "
        f"{RELAY_PLUGINS_CONFIG_ENV} in .env).\n"
        "# Generated by `hermes update` from the legacy TINO_NEMO_RELAY_ATIF_*/ATOF_* variables,\n"
...
def print_relay_migration_report(results: list[RelayMigrationResult]) -> None:
    ...
    if migrated:
        ...
        print("   The legacy TINO_NEMO_RELAY_ATIF_*/ATOF_* variables stopped producing traces after the")

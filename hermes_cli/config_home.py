"""Directory initialization and storage diagnostics for the active Tino home."""
...
def _ensure_directory(path: Path, *, create: bool, secure: bool, home: Path) -> None:
    ...
    try:
        ...
        raise HomeInitializationError(
            f"Cannot initialize Tino directory {path}: {exc}. "
            ...
            "Tino has not replaced the link or created its missing target."
        ) from exc
...
def initialize_home(home: Path, subdirs: tuple[str, ...], ensured: set[str]) -> None:
    ...
    try:
        ...
        try:
            ...
            raise HomeInitializationError(
                f"Cannot initialize Tino home {home}: {exc}. "
...
def config_load_issue(exc: Exception):
    ...
    if isinstance(exc, (HomeInitializationError, OSError)):
        return ConfigIssue(
            "error", f"Tino storage is unavailable: {exc}",

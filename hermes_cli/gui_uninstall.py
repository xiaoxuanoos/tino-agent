"""Tino Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for an app named "Tino" on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Tino"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "Tino"
    return _env_dir("XDG_CONFIG_HOME", home / ".config") / "Tino"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    return [desktop_entry_path(), data_base / "applications" / "Tino.desktop"] + [
...
def agent_is_installed(hermes_home: Path) -> bool:
    """True when a usable Python agent install exists under TINO_HOME (gates the desktop UI's options).
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so Tino

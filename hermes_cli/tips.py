...
TIPS = [
    ...
    "Ctrl+Z suspends Tino to the background — run fg in your shell to resume.",
    ...
    "hermes mcp serve runs Tino itself as an MCP server for other agents.",
    ...
    "hermes backup creates a zip backup of your entire Tino home directory.",
    ...
    "hermes gateway install sets up Tino as a system service (systemd/launchd).",
    ...
    "TINO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Tino tools programmatically — results stay out of context.",
    ...
    "When exiting, Tino prints a resume command with session ID and stats.",
    ...
    # --- Gateway & Messaging ---
    "Tino runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to Tino via a local macOS server.",
    ...
    "Tino loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "hermes acp runs Tino as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "TINO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Tino your own.",
    ...
    "Before a gateway session reset, Tino auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope Tino state via TINO_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "TINO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in TINO_HOME and the host CLI auto-execs into the container.",
    ...
    "TINO_DEV=1 bypasses container mode detection for local development.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Tino auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Tino config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Tino is working.',
    ...
    "TINO_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    ...
    # --- TUI & Dashboard ---
    'TINO_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "TINO_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'hermes dashboard embeds the full Tino TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'TINO_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'TINO_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'TINO_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'TINO_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    'hermes import restores a full Tino backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    ...
    'TINO_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'TINO_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hermes/plugins/ — trust-gated by design.',
    "TINO_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'TINO_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'TINO_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'TINO_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'TINO_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'TINO_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'TINO_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'TINO_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'TINO_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'TINO_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'TINO_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Tino block commands when the tirith scanner itself errors out.',
...
# Task-oriented example prompts for the empty composer. Kept generic — Tino is

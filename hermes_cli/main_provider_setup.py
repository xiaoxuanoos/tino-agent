...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        _say("", "  Auxiliary models — side-task routing", "",
             ...
             "  Tino only falls back to a lightweight backend (OpenRouter,",
...
_CUSTOM_API_MODES = (
    ("", "Auto-detect", "Use Tino URL heuristics; best for standard OpenAI-compatible endpoints.",
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (is_claude_code_token_valid(creds) or bool(creds.get("refreshToken"))):
            ...
            print(f"    Tino will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False

...
def _print_banner(title: str) -> None:
    ...
    print()
    rule = "─" * 57
    for line in (f"┌{rule}┐", f"│          ☤ Tino — {title:<35s}│", f"└{rule}┘"):
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    if running and _warn_running(
        ...
        ("Messaging platforms (Telegram, Discord, Slack) only allow one "
         ...
         "Tino may try to use the same token, causing disconnects.",
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Warn if a Tino gateway has connected platforms (token conflicts, e.g. Telegram 409)."""
    ...
    if connected and _warn_running(
        auto_yes, "Tino gateway is running with active connections: " + ", ".join(connected), [],
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: hermes claw <command> [options]\n\nCommands:\n"
              "  migrate          Migrate settings from OpenClaw to Tino\n"
              "  cleanup          Archive leftover OpenClaw directories after migration\n\n"
              "Run 'hermes claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Tino migration: preflight, preview, confirm, back up, apply."""
    ...
    if not opts.no_backup:
        try:
            ...
            return _error_block(
                ...
                "Re-run with --no-backup to skip, or free up disk space under the Tino home.",

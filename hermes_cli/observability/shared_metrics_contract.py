"""Bounded product contract for the first Tino shared-metrics slice."""
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Tino installation method."""
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Tino logical scope."""
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Tino terminal state to bounded (outcome, end_reason, termination)."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Tino registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Tino tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Tino."""

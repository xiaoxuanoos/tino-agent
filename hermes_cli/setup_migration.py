...
# platforms; config values and instruction/context .md files may not map 1:1 to Tino.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Tino to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Tino at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Tino at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Tino at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Tino at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Tino equivalents",
...
_MIGRATION_WARNING_NOTES = (
    "  Note: OpenClaw config values may have different semantics in Tino.",
    "  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Tino's yolo mode.",
...
def _print_migration_preview(report: dict):
    ...
    groups = (
        ...
        ("conflict", "  Would overwrite (conflicts with existing Tino config):", Colors.YELLOW,
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    _info(f"Found OpenClaw data at {openclaw_dir}",
          "Tino can preview what would be imported before making any changes.", None)
    ...
    # overwrite=False so existing Tino configs are preserved. The user saw the preview;
    ...
    for key, printer, text in (
        ...
        ("conflict", print_info,
         "Skipped {n} item(s) that already exist in Tino (use hermes claw migrate --overwrite to force)."),

...
if len(sys.argv) == 2:
    with tempfile.TemporaryDirectory(prefix="kanban-transport-") as home:
        env = {"HOME": home, "TINO_HOME": home + "/hermes", "PATH": os.environ["PATH"], "PYTHONDONTWRITEBYTECODE": "1"}
...
os.environ.update({"TINO_KANBAN_DB": str(db), "TINO_KANBAN_BOARD": "default", "TINO_KANBAN_TASK": own, "TINO_KANBAN_RUN_ID": str(task.current_run_id), "TINO_KANBAN_CLAIM_LOCK": task.claim_lock})
ch = home / "codex"
ch.mkdir()
...
with CodexAppServerClient(codex_bin=shutil.which("codex") or "codex", codex_home=str(ch)) as c:
    try:
        ...
        raise
    script = "import os,sys,json;sys.path.insert(0," + repr(str(repo)) + ");from tools import kanban_tools as kt;print(json.dumps({'marker':os.getenv('TINO_DELEGATED_CHILD_CONTEXT'),'db':os.getenv('TINO_KANBAN_DB'),'mutation':json.loads(kt._handle_complete({'task_id':" + repr(own) + ",'summary':'must refuse'}))}))"

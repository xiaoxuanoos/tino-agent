...
# Needs: uv on PATH, a Python for the venv. Runs with a disposable HOME/TINO_HOME, no npm
...
uv venv -q --python "${TINO_AB_PYTHON:-3.11}" "$ROOT/install/.venv"
uv pip install -q --python "$ROOT/install/.venv/bin/python" -e "$ROOT/install"

export HOME="$ROOT/home" TINO_HOME="$ROOT/home/.hermes"
mkdir -p "$TINO_HOME"
...
python3 - "$TINO_HOME/logs/update_receipts/latest.json" <<'PY'
import json, sys
try:

...
os.environ.clear()
os.environ.update(
    HOME=home, TINO_HOME=home + "/.hermes", PATH="/usr/bin:/bin", NO_PROXY="*"
)
os.chdir(home)
...
assert Path(os.environ["TINO_HOME"]).is_relative_to(Path(os.environ["HOME"]))
...
Path(os.environ["TINO_HOME"]).mkdir(parents=True, exist_ok=True)
# JSON is valid YAML, allowing the real config loader to read the seeded file.
(Path(os.environ["TINO_HOME"]) / "config.yaml").write_text(json.dumps(config))
    ...
    json.dumps(
        {
            ...
            "hermes_home": os.environ["TINO_HOME"],
            "captures": captures,
            "rows": rows,

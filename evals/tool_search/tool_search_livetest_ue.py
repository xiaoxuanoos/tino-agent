...
probe date 2026-07-02) into the Tino tool registry with mock handlers,
...
def run_one(scenario, mode, scale, rep, out_dir: Path):
    ...
    os.environ["TINO_HOME"] = str(hermes_home)
    ...
    shutil.rmtree(Path(os.environ["TINO_HOME"]).parent, ignore_errors=True)
    return rec

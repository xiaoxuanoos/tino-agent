...
test.beforeAll(async () => {
  ...
  env = buildAppEnv(sandbox, { HOME: sandbox.root, TINO_DESKTOP_PYTHON: python,
    ...
    PYTHONPATH: repo, TINO_SINGLE_QUERY_LINGER_SECONDS: '1' })
...
test('ordinary cron pins an unowned named target under a custom root', async () => {
  ...
  const page = fixture.page
  const composer = await openBot('Tino')
  ...
  const discovery = 'from pathlib import Path; import json,os; from tools.bot_live_delivery import find_canonical_owner; h=Path(os.environ["TINO_HOME"]); print(json.dumps({"default":find_canonical_owner(h),"alpha":find_canonical_owner(h/"profiles"/"alpha")}))'
  ...
  try {
    ...
    const rows = probe('import sqlite3,json,os; from pathlib import Path; h=Path(os.environ["TINO_HOME"]); print(json.dumps({n:sqlite3.connect(h/"state.db" if n=="default" else h/"profiles"/n/"state.db").execute("select session_id,role,content from messages").fetchall() for n in ["default","alpha"]}))')

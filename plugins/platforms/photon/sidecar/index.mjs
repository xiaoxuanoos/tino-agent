// Tino Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Tino): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Tino -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Tino-Sidecar-Token: ${TOKEN}`):
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Tino can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Tino'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Tino allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Tino, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-tino-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Tino supplied them so a known-good

...
Small Node helper that bridges Tino Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Tino is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Tino-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Tino can receive inbound messages
...
The Tino plugin's `hermes photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Tino is Python, so replies go through this sidecar

...
def _build_client(openai: Any, base_url: str, api_key: str) -> Any:
    """``openai.OpenAI`` on Tino's env-only-proxy httpx client, so a local/custom endpoint never
...
class OpenAIImageGenProvider(StaticImageGenProvider):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # gpt-image bills per text/image token; the tier id is a Tino label, the API model prices.

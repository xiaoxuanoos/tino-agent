...
One active meeting at a time, recorded in ``$TINO_HOME/workspace/meetings/.active.json``
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Tino Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "TINO_MEET_URL": url, "TINO_MEET_OUT_DIR": str(out),
           "TINO_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "TINO_MEET_HEADED"),
        (auth_state, "TINO_MEET_AUTH_STATE"),
        (duration, "TINO_MEET_DURATION"),
        (mode, "TINO_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "TINO_MEET_REALTIME_MODEL"),
        (realtime_voice, "TINO_MEET_REALTIME_VOICE"),
        (realtime_instructions, "TINO_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("TINO_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["TINO_MEET_REALTIME_KEY"] = realtime_api_key

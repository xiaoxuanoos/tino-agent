...
def _setup_identity_mapping(cfg: dict, hermes_host: dict, current_peer: str, new_host: bool) -> None:
    ...
    if gw_platforms:
        ...
        notice, question = (
            ("\n  Each gateway account (a Telegram user, a Discord user, ...)\n"
             ...
             "Running the Tino gateway (Telegram/Discord/etc.)? (y/N)") if gw_platforms is None else
    ...
    print("\n  This step covers the HUMAN mapping only. Each account using the\n"
          ...
          "  Tino profile brings its own AI peer to the gateway.\n"
...
def _setup_wizard(args) -> None:
    ...
    print(f"\nHoncho memory setup\n{RULE}\n  Honcho gives Tino persistent cross-session memory.\n  Config: {write_path}")
...
def _active_profile_name() -> str:
    """Active Tino profile name (respects --target-profile override)."""
    if _profile_override:
...
def _classify_workspace_peers(
    ...
    # Host blocks that are not Tino profiles: other apps sharing the config.
...
{RULE}
  ...
  AI peer:     {_pref(hermes, cfg, 'aiPeer') or _host_key()}
    Tino's identity in Honcho. Seed with 'hermes honcho identity <file>'.
...
{RULE}
  ...
  Dialectic   {_pref(hermes, cfg, 'dialecticMaxChars') or 600} chars, reasoning: {_pref(hermes, cfg, 'dialecticReasoningLevel') or 'low'}
    AI-to-AI inference. Tino asks Honcho's AI peer a question
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Tino + Honcho."""
    ...
    print("\nHoncho migration: OpenClaw native memory → Tino\n" + "─" * 50)
    print("""
...
""")
    if has_key:
        ...
        print("""  Honcho is a cloud memory service that gives Tino persistent memory
...
""")
  ...
  (Tino calls migrate_memory_files() on first session init.)
...
Step 4  Seed AI identity files → Honcho AI peer
  ...
  In Tino, they are seeded once into Honcho's AI peer through the
...
Step 5  What changes vs. OpenClaw native memory
  ...
  Session naming
    ...
    Tino:   per-session by default — each run gets its own session
...
_SUBCOMMANDS = (
    ...
    ("migrate", "Step-by-step migration guide from openclaw-honcho to Tino Honcho", cmd_migrate, ()),

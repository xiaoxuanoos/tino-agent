...
# $TINO_HOME — auth.json, .env, config.yaml — ends up root-owned and
...
# returns "Provider authentication failed: Tino is not logged into Nous
...
# $TINO_HOME is uid-aligned with the supervised processes. When invoked
...
#
# Opt-out: set TINO_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
case "${TINO_DOCKER_EXEC_AS_ROOT:-}" in
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "hermes-shim: re-run with --user hermes or set TINO_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

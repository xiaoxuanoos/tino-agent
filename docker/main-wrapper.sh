...
# with-contenv before touching TINO_HOME / PATH. On the non-PID-1
...
if [ -z "${TINO_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${TINO_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export TINO_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset TINO_MAIN_WRAPPER_ENV_READY
...
_hermes_orig_cwd="${TINO_ORIG_CWD:-$PWD}"

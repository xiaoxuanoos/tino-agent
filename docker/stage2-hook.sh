...
# $TINO_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass TINO_UID/TINO_GID — or the PUID/PGID aliases — which the
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e TINO_UID=\$(id -u) -e TINO_GID=\$(id -g) ...
...
# --- Bootstrap TINO_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid hermes mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom TINO_HOME paths whose parents only
# root can create (e.g. `TINO_HOME=/home/hermes/.hermes` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for TINO_UID/TINO_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  TINO_UID/
# TINO_GID still win when both are set.  See #15290, salvages #25872.
TINO_UID="${TINO_UID:-${PUID:-}}"
TINO_GID="${TINO_GID:-${PGID:-}}"

if [ -n "${TINO_UID:-}" ] && validate_uid_gid "$TINO_UID" && [ "$TINO_UID" != "$(id -u hermes)" ]; then
    echo "[stage2] Changing hermes UID to $TINO_UID"
    usermod -u "$TINO_UID" hermes
fi
if [ -n "${TINO_GID:-}" ] && validate_uid_gid "$TINO_GID" && [ "$TINO_GID" != "$(id -g hermes)" ]; then
    echo "[stage2] Changing hermes GID to $TINO_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$TINO_GID" hermes 2>/dev/null || true
fi
...
# When TINO_UID is remapped or the top-level $TINO_HOME isn't owned by
# the runtime hermes UID, restore ownership to hermes — but ONLY for the
# directories hermes actually writes to. The full $TINO_HOME may be a
...
needs_chown=false
if [ "$(stat -c %u "$TINO_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $TINO_HOME (targeted) to hermes ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $TINO_HOME: chown the directory itself (not its contents)
    # so hermes can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$TINO_HOME"; then
        :
    else
        chown hermes:hermes "$TINO_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $TINO_HOME failed (rootless container?) — continuing"
    fi
    # Tino-owned subdirs: recursive chown is safe here because these are
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$TINO_HOME/$sub" ] && tree_has_non_hermes_owner "$TINO_HOME/$sub"; then
            chown_hermes_tree "$TINO_HOME/$sub"
        fi
    done
fi
...
# $TINO_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# TINO_DISABLE_LAZY_INSTALLS=1. Keeping /opt/hermes root-owned and
...
# $TINO_HOME/lazy-packages dir on the data volume (Dockerfile sets
# TINO_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# Always reset ownership of $TINO_HOME/profiles to hermes on every
...
if [ -d "$TINO_HOME/profiles" ] && tree_has_non_hermes_owner "$TINO_HOME/profiles"; then
    chown_hermes_tree "$TINO_HOME/profiles"
fi

# Always reset ownership of $TINO_HOME/cron on every boot for the same
...
# gate as profiles/).
if [ -d "$TINO_HOME/cron" ] && tree_has_non_hermes_owner "$TINO_HOME/cron"; then
    chown_hermes_tree "$TINO_HOME/cron"
fi
...
# top-level $TINO_HOME is mis-owned, so a warm volume with hermes-owned
# TINO_HOME but root-owned logs/gateways would otherwise leave
...
if [ -d "$TINO_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$TINO_HOME/logs/gateways"; then
        :
    else
        chown hermes:hermes "$TINO_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# data-volume chown above only runs when the top-level $TINO_HOME is
...
# and cron/.
if [ -d "$TINO_HOME/platforms/pairing" ] && tree_has_non_hermes_owner "$TINO_HOME/platforms/pairing"; then
    chown_hermes_tree "$TINO_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$TINO_HOME/pairing" ] && tree_has_non_hermes_owner "$TINO_HOME/pairing"; then
    chown_hermes_tree "$TINO_HOME/pairing"
fi
...
# *subdirectories*; loose state files living directly under $TINO_HOME
...
# sweep so host-owned files in a bind-mounted $TINO_HOME are never
...
for f in \
    ...
    active_profile; do
    if [ -e "$TINO_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$TINO_HOME/$f"; then
            :
        else
            chown hermes:hermes "$TINO_HOME/$f" 2>/dev/null || true
        fi
    fi
done
...
if [ -f "$TINO_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$TINO_HOME/config.yaml"; then
        :
    else
        chown hermes:hermes "$TINO_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$TINO_HOME/config.yaml" 2>/dev/null || true
    fi
fi
...
# shell isn't a second interpreter — defends against $TINO_HOME values
...
as_hermes mkdir -p \
    ...
    "$TINO_HOME/home" \
    "$TINO_HOME/pairing" \
    "$TINO_HOME/platforms/pairing" \
    "$TINO_HOME/lazy-packages"
...
# $TINO_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $TINO_HOME: it is a shared DATA volume, commonly
...
# stale 'docker' stamp from $TINO_HOME if one is present (the host install's
...
if [ -f "$TINO_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$TINO_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$TINO_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$TINO_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$TINO_HOME/$dest"; then
            :
        else
            as_hermes cp "$INSTALL_DIR/$src" "$TINO_HOME/$dest"
        fi
    fi
}
...
# not generate one. Tino loads $TINO_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$TINO_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$TINO_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $TINO_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$TINO_HOME/.env" ] && ! refuse_symlinked_path "clean" "$TINO_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$TINO_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$TINO_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$TINO_HOME/.env"; then
        :
    else
        if [ ! -f "$TINO_HOME/.env" ]; then
            ...
            (umask 077 && as_hermes touch "$TINO_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$TINO_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$TINO_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$TINO_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $TINO_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $TINO_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi

# --- Sync deploy-injected Nous routing overrides into every profile .env ---
# Under multiplex, hermes_cli.auth_nous reads TINO_PORTAL_BASE_URL (or its
...
# carries the value into $TINO_HOME/.env and every profiles/*/.env: the
...
sync_routing_overrides() {
    ...
    for _name in TINO_PORTAL_BASE_URL NOUS_PORTAL_BASE_URL NOUS_INFERENCE_BASE_URL; do
        eval "_value=\${$_name:-}"
...
sync_routing_overrides "$TINO_HOME/.env"
for _profile_dir in "$TINO_HOME"/profiles/*/; do
...
if [ -f "$TINO_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$TINO_HOME/.env"; then
        :
    else
        chown hermes:hermes "$TINO_HOME/.env" 2>/dev/null || true
        chmod 600 "$TINO_HOME/.env" 2>/dev/null || true
    fi
fi
...
# $TINO_HOME on the mounted volume. Run the same safe, non-interactive
...
# Set TINO_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$TINO_HOME/config.yaml" ]; then
...
if [ ! -f "$TINO_HOME/auth.json" ] && [ -n "${TINO_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$TINO_HOME/auth.json"; then
        :
    else
        printf '%s' "$TINO_AUTH_JSON_BOOTSTRAP" > "$TINO_HOME/auth.json"
        chown hermes:hermes "$TINO_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$TINO_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via TINO_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$TINO_HOME/auth.json" ] && [ -n "${TINO_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$TINO_HOME/auth.json"; then
        :
    else
...
# can set TINO_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# TINO_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$TINO_HOME/gateway_state.json" ] && \
        [ "${TINO_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$TINO_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$TINO_HOME/gateway_state.json"
        chown hermes:hermes "$TINO_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$TINO_HOME/gateway_state.json"
    fi
fi
...
# directory. agent-browser (the runtime CLI Tino spawns for the

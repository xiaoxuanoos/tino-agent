...
/**
 * User widget apps — Tino authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$TINO_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.hermes/plugins/`: files under TINO_HOME execute
...
const widgetsDir = () => join(process.env.TINO_HOME?.trim() || join(homedir(), '.hermes'), 'tui-widgets')
 ...
 *  every change, so a widget Tino writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Tino) ever writes must hot-load too — a 10s poll here read

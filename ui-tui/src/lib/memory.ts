...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because TINO_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.TINO_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.hermes', 'heapdumps')
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.TINO_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.TINO_HEAPDUMP_MAX_BYTES?.trim()

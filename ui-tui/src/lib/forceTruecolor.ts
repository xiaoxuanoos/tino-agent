...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.TINO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Tino on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via TINO_TUI_TRUECOLOR=1.

...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into TINO_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.TINO_TUI_THEME || env.TINO_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.TINO_TUI_BACKGROUND
  ) {
    env.TINO_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.TINO_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.TINO_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into TINO_TUI_THEME.

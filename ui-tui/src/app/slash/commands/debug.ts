...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $TINO_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see TINO_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['TINO_TUI_BACKGROUND', process.env.TINO_TUI_BACKGROUND ?? '(unset)'],
            ['TINO_TUI_THEME', process.env.TINO_TUI_THEME ?? '(unset)'],

...
Usage:
  ...
  <server_name> matches the files in $TINO_HOME/mcp-tokens/<server>.json etc.
...
def _hermes_home():
    # Prefer Tino' own resolver (profile-safe); fall back to env then ~/.hermes.
    try:
        from hermes_constants import get_hermes_home
        return str(get_hermes_home())
    except Exception:
        return os.environ.get("TINO_HOME") or os.path.expanduser("~/.hermes")

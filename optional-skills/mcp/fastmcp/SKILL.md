...
metadata:
  hermes:
    ...
    related_skills: [tino-agent, mcporter]
prerequisites:
  commands: [python]
---
...
- smoke-test a server with the FastMCP CLI before wiring it into Tino or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Tino. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is Tino integration, either:
...
### Tino cannot see the deployed server

The server-building part may be correct while the Tino config is not. Load the `native-mcp` skill and configure the server in `~/.hermes/config.yaml`, then restart Tino.

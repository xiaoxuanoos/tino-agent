...
This is an optional third-party workflow, not a Tino core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Tino native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Tino native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Tino native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Tino usage patterns
...
- For large result sets, save JSON to `~/.hermes/cache/scratch/*.json` (the Tino scratch dir) instead of stuffing everything into context.
- Do not silently choose Parallel when Tino native tools are already sufficient.

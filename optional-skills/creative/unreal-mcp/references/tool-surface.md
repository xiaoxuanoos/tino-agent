...
Under Tino that means each tool becomes `mcp_unreal_engine_<tool_name>` at
...
| Server Port Number | `8000` | Change on conflict; mirror in Tino config url |
...
| `ModelContextProtocol.GenerateClientConfig <Client\|All>` | Write client config files (ClaudeCode/Cursor/VSCode/Gemini/Codex) — NOT used for Tino |
...
- **After Live Coding / authoring:** connected clients can hold stale
  schemas. `ModelContextProtocol.RefreshTools`, then reconnect (new Tino
...
then re-`list_toolsets` from Tino. Users on Claude Code can scaffold with

...
Companion skill for the `unreal-engine` entry in the Tino MCP catalog. The
...
Two halves, in this order: the editor side must be up before Tino connects.
...
Do NOT use `ModelContextProtocol.GenerateClientConfig` for Tino — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Tino connects
...
2. Start the Tino session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Tino
they appear as:
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Tino runs batched calls
...
- **Start order matters.** Editor + server up first, then the Tino
...
- **Timeouts on long operations.** Tino's per-call default is 120 s; asset

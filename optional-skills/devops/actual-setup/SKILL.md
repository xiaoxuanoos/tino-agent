---
name: actual-setup
description: Set up Actual Computer (actual.inc) inference in Tino.
version: 2.0.0
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Tino inference
...
- User has an `ac_` key and wants Tino routed through their Actual cluster.
...
- Tino has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Tino — the
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Tino at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
...
1. **reasoning_effort trap (handled by Tino since the first-class provider).**
   ...
   request still 400s this way on an old Tino, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Tino's default toolset
   ...
   servers emit a bare `data: [DONE]` — Tino reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Tino the built-in provider wins the name; stale custom blocks

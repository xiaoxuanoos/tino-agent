...
Usage (via cron with --no-agent):
    ...
    hermes cron create hermes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$TINO_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hermes-issues --repo NousResearch/hermes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Tino .env file
(``${TINO_HOME:-~/.hermes}/.env``) to avoid the 60 req/hr

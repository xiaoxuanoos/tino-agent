...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("TINO_HOME", os.path.expanduser("~/.hermes")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("TINO_HOME", Path.home() / ".hermes")) / "skills" / "red-teaming" / "godmode"
...
TINO_HOME = Path(os.getenv("TINO_HOME", Path.home() / ".hermes"))
CONFIG_PATH = TINO_HOME / "config.yaml"
PREFILL_PATH = TINO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Tino models — already uncensored, just needs clean prompt
    "hermes": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _get_current_model() -> tuple:
    """Read current model and provider from Tino config.yaml.
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Tino for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Tino for changes to take effect.")

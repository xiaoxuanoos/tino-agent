...
- Skill helper paths:
  - `${TINO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${TINO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${TINO_SKILL_DIR}/scripts/publish.sh"
...
```bash
DRIVE="${TINO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default

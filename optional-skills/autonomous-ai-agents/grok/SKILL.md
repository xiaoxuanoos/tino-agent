...
version: 0.1.1
author: Matt Maximo (MattMaximo), Tino Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, tino-agent]
---

# Grok Build CLI — Tino Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Tino terminal. Grok
...
1. Prepare stable input files first with Tino tools (`read_file`,
...
2. **Don't conflate Tino's xAI auth with the `grok` CLI's auth.** Tino's
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Tino's xAI auth carries
   over.

...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Tino Agent
license: MIT
platforms: [linux, macos]
metadata:
  hermes:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, tino-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Tino.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Tino-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `tino-agent` (Tino subagents via `delegate_task`).

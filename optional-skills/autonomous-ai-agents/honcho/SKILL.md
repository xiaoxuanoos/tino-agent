---
name: honcho
description: Configure and troubleshoot Honcho memory for Tino.
version: 2.0.0
author: Tino Agent
license: MIT
...
# Honcho Memory for Tino

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Tino profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this Tino profile's AI peer
...
Honcho models conversations as interactions between **peers**. Tino creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Tino instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Tino run |
...
Each Tino profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for Tino when Honcho memory is active.
...
Config file: `$TINO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
| `hermes honcho sync` | Create host blocks for all Tino profiles that don't have one yet |
| `hermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Tino + Honcho |

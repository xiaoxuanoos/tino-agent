...
description: Import an OpenClaw setup (memories, skills) into Tino.
version: 1.0.0
author: Tino Agent (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [Migration, OpenClaw, Tino, Memory, Persona, Import]
    related_skills: [tino-agent]
---

# OpenClaw -> Tino Migration

Use this skill when a user wants to move their OpenClaw setup into Tino Agent with minimal manual cleanup.
...
- import `SOUL.md` into the Tino home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Tino memory entries
- merge OpenClaw command approval patterns into Tino `command_allowlist`
- migrate Tino-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Tino working-directory configuration
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Tino workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct Tino destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of Tino-compatible secrets, currently:
...
Tino CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Tino. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Tino workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Tino skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Tino unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Tino targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Tino destination. Unsupported auth blobs must still be reported as skipped.
...
- Tino persona state imported
- Tino memory files populated with converted OpenClaw knowledge

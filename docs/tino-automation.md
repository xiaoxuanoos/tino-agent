...
这套自动化不会在当前目录或原 Hermes 仓库自动运行。当前 `origin` 仍指向
`NousResearch/hermes-agent`，而且工作区有大量未提交文件；在专属 Tino 仓库建立并审查
变更前，不能启用自动推送。云端示例只用于 SSH 隧道访问的单用户灰度，**不支持公众注册**。
...
还需要专属发行仓库与发布凭据，不能让这份脏工作树直接拉取原 Hermes 更新。

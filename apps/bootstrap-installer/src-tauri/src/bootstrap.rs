...
//! TINO_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for TINO_HOME. Tests use this; production
...
/// Spawn the locally-built Tino desktop binary, then close the installer
...
pub async fn launch_hermes_desktop(
    ...
    let exe_path = resolve_hermes_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Tino desktop at {}. The desktop build step \
    ...
    tracing::info!(?exe_path, "launching Tino desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Tino
...
pub(crate) fn resolve_hermes_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Tino.exe"),
            ("win-arm64-unpacked", "Tino.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Tino.app/Contents/MacOS", "Tino"),
            ("mac-arm64/Tino.app/Contents/MacOS", "Tino"),
        ]
    } else {
...
/// path so a bare re-open just opens Tino instead of re-running setup.
pub(crate) fn hermes_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".tino-bootstrap-complete").exists()
...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    let tmp_path = install_root.join(".tino-bootstrap-complete.tmp");
    {
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hermes_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Tino desktop app")
    })?;
...
async fn run_bootstrap(
    ...
    // Copy ourselves to TINO_HOME/hermes-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_hermes_home() {
        tracing::warn!(?err, "failed to copy installer into TINO_HOME (non-fatal)");
        emit_log(&format!(
...
mod tests {
    ...
    #[cfg(windows)]
    const STDIO_HELPER_ENV: &str = "TINO_BOOTSTRAP_STDIO_HELPER";
    #[cfg(windows)]
    const STDIO_SLEEPER_ENV: &str = "TINO_BOOTSTRAP_STDIO_SLEEPER";
    #[cfg(windows)]
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Tino"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Tino.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Tino.exe");
    ...
    // what the updater ditto's over /Applications/Tino.app). A regression in
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".tino-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".tino-bootstrap-complete");
        let tmp_path = root.join(".tino-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
    ...
    fn hermes_is_installed_treats_marker_existence_as_sufficient() {
        ...
        make_release_tree(&root);
        std::fs::write(root.join(".tino-bootstrap-complete"), b"").unwrap();
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".tino-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".tino-bootstrap-complete.tmp").exists(),

...
test('405 method-not-allowed toasts a restart in plain words with a Restart Tino action', () => {
  ...
  expect($notifications.get()[0]?.action?.label).toBe(en.notifications.actions.restartTino)
  $notifications.get()[0]?.action?.onClick()

 ...
 * the raw Cloud hostname, or as generic "Tino" when Cloud was the only
 * source.
 *
...
describe('alias identity survives the hosted handoff (#89131)', () => {
  ...
  it('renders the sole Cloud-only default as the alias, not "Tino"', () => {
  ...
  it('never leaks the alias onto a same-named default on another connection', () => {
    ...
    // Local default while the ACTIVE gateway is local: untouched "Tino".
    expect(displayName({ name: 'default' }, null)).toBe('Tino')
  })

...
describe('reasoning blocks in streamed markdown', () => {
  it('does not fuse the words a stripped block separated', () => {
    expect(preprocessMarkdown('no<thinking>hmm</thinking> Tino Desktop')).toBe('no Tino Desktop')
    ...
    expect(preprocessMarkdown('no<thinking>a</thinking><think>b</think>Tino')).toBe('no Tino')
  })

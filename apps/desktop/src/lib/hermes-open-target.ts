 ...
 * `tino://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `tino://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `tino://open/my-page?item=x` → `/my-page?item=x` (generic open)
...
export function normalizeHermesOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('tino://') || trimmed.startsWith(`${TINO_PROTOCOL}//`)) {
    try {
      ...
      // tino://index-network/intent/1 → /index-network/intent/1
 ...
 * (`tino://<kind>/<name>?…` → kind/name/params).
 */
...
/** Convenience for `tino://open/<name>?…` payloads. */

...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      // Tino Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new TINO_HOME (reloads the window). Pass null to
       ...
       *  (TINO_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only fact the
       *  main process also stamps onto every backend it spawns. */
      guestOnboardingEnabled?: boolean
      /** Launch flag: skip the first-run film (TINO_SKIP_INTRO=1 or
       *  --skip-intro) so a fresh TINO_HOME lands on the guided chat. */
      skipIntro?: boolean
      ...
      // Local Desktop runtime-plugin root (<TINO_HOME>/desktop-plugins),
      ...
      /** LOCAL `<TINO_HOME>/logs` (profile-aware) — error card "Open Logs". */
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Tino Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Tino Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Tino Cloud org (slug or id) to
...
// A discovered Tino Cloud agent — the trimmed DTO from NAS GET /api/agents.

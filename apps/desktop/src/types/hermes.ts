...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Tino doesn't know how to remove it. */
...
export interface UsageStats {
  /** Mean model-call latency over the last ~10 calls, when supplied. */
...
// non-desktop) Tino simply carries none of it. See store/profile-share.ts.
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Tino, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Tino but install on demand. */
 ...
 *  not Tino); elsewhere, driver health from `cua-driver doctor`. `null`

 ...
 * product name (`Tino`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Tino'
 *
 ...
 * (the mac-unpacked Tino.app bundle) and Windows (win-unpacked), so we
 ...
 * Tino.exe that Windows can't load (truncated PE from a corrupt cached
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Tino.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Tino'}.exe`

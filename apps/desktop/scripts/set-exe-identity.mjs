#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Tino icon + version metadata onto the
// built Tino.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Tino.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-Tino.exe>
//
...
// Stamp the Tino icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(
  ...
  const options = {
    icon,
    'version-string': {
      ProductName: 'Tino Agent',
      FileDescription: 'Tino Agent',
  ...
  console.log('[set-exe-identity] done — Tino Agent icon + identity stamped')
}

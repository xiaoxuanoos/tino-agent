...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Tino.app/Contents/MacOS/Tino', 'darwin'),
    '/Applications/Tino.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Tino.app/Contents/MacOS/Tino', 'darwin'),
    '/Users/x/Applications/Tino.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  ...
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\hermes-desktop\\Tino.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\hermes-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Tino.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_HermesXXXX/hermes', 'linux', { APPIMAGE: '/home/x/Apps/Tino.AppImage' }),
    '/home/x/Apps/Tino.AppImage'
  )
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Tino.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Tino.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  ...
  assert.match(script, /export TINO_HOME='\/home\/x\/\.hermes'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hermes',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Tino',
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Tino" >nul 2>&1/)

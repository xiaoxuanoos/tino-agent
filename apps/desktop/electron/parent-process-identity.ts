export type ParentWatchdogEnv = {
  TINO_PARENT_PID: string
  TINO_PARENT_START_MARKER?: string
  TINO_PARENT_NONCE?: string
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { TINO_PARENT_PID: String(pid) }
  env.TINO_SPAWN = spawnTag(pid, startMarker)
  ...
  env.TINO_PARENT_START_MARKER = startMarker
  env.TINO_PARENT_NONCE = nonce

  return env
}

/** Build the `TINO_SPAWN` tag mirrored by hermes_cli/process_identity.py.

...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Tino gateway uses OAuth, but you are not signed in. ' +
...
/**
 * True when the backend URL points at a Nous-managed Tino Cloud instance
...
export async function waitForHermesReady(baseUrl: string, options: HermesReadyOptions): Promise<void> {
  ...
  throw new Error(`Tino backend did not become ready: ${detail}`)
}

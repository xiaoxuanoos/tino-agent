 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Tino Agent by spawning
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass TINO_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          TINO_HOME: hermesHome || process.env.TINO_HOME || ''
        }
      })
    )

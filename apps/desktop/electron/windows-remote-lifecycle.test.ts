...
test('Windows probe validates Tino and Python topology before selection', async () => {
  let script = ''
  await probeWindowsRemote(
    sshWith(async command => {
      script = Buffer.from(command.split(' ').at(-1) || '', 'base64').toString('utf16le')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Tino is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Tino is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Tino's\\python.exe" }, 'inspect', [
    'C:\\x y\\hermes.exe'
  ])

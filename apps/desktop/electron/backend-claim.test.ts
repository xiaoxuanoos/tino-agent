...
test('exit line carries the buffered tail next to the exit code, preferring the signal', () => {
  ...
  assert.equal(
    formatBackendExitLine('Ignoring stale Tino backend exit', 1, null, tail),
    'Ignoring stale Tino backend exit (1)\nRecent backend output:\nTraceback (most recent call last):'
  )
  assert.equal(formatBackendExitLine('Tino backend exited', null, 'SIGTERM', tail), 'Tino backend exited (SIGTERM)\nRecent backend output:\nTraceback (most recent call last):')
})

test('exit line stays byte-identical to the legacy shape when the tail is empty or missing', () => {
  assert.equal(formatBackendExitLine('Tino backend exited', 0, null, createBackendOutputTail(64)), 'Tino backend exited (0)')
  assert.equal(formatBackendExitLine('Tino backend exited', 1, null, null), 'Tino backend exited (1)')
})

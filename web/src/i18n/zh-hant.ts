...
export const zhHant: Translations = {
  ...
  pluginsPage: {
    ...
    headline:
      "探索、安裝、啟用並更新 Tino 外掛（對齊 `hermes plugins` CLI）。",
  ...
  env: {
    ...
    hideValue: "隱藏值",
    customTitle: "自訂密鑰",
    customHint: "儲存在 .env 中、Tino 無法識別的任意環境變數。可用於為技能、MCP 伺服器或你自己的工具注入環境變數。",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "從真實工作階段歷史中獲得的 Tino 可收集徽章。已知尚未達成的成就會顯示為「已發現」；秘密成就在首次出現相符行為之前保持隱藏。",
      scan_subtitle:
        "正在掃描 Tino 工作階段歷史。在歷史紀錄較多時，首次掃描可能需要 5–10 秒。",
    },
    actions: {
      rescan: "重新掃描",
    },
    ...
    guide: {
      ...
      secret_header: "秘密成就",
      secret_body:
        "秘密成就會隱藏其確切觸發條件。一旦 Tino 偵測到相關訊號，卡片便會變為「已發現」並顯示其需求。",
      scan_status_header: "掃描狀態",
      scan_status_body:
        "Tino 正在對本機歷史進行一次掃描，之後卡片會自動出現。即使需要幾秒鐘，也並未卡住。",
